...
begin
    if rgArea.itemindex = 0 then begin
        ...
        tabIOP.TabVisible := False;
        pageControl1.ActivePageIndex := 0;
        mmLog.Lines.Add(#13#10+ DatetimeToStr(now)+'   >>> Log elaborazioni su DBTecnico');
    end
    else begin
        ...
        mmLog.Lines.Add(#13#10+ DatetimeToStr(now)+'   >>> Log elaborazioni su IOP files');
    end;
end;
...
begin
    ...
    if length(arrCodici) = 0 then begin
        ...
        if rgArea.itemindex = 0 then
            MessageDlg('Necessario aprire lista Codici Prodotti !', mtWarning, [mbOk], 0, mbOk)
        else
