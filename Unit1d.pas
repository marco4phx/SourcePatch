    ...
    *)
    ...
    for i := 0 to lviewProdotti.Items.Count -1 do begin
        ...
        if btnStop.tag >0 then begin
            mmLog.Lines.Add('Elebarazione Interrotta !'+#13#10 );
            exit;
        end;
        ...
        if lviewProdotti.Items[i].Checked  then begin
            ...
            if FindFile( fToFind ) then begin   // '_DSP.html'
                ...
                if nPatch > 0 then begin
                    ...
                    if not DirectoryExists(tPath) then begin
                        //if not CreateDir('C:\temp') then
                        //  raise Exception.Create('Cannot create C:\temp')
                        mmLog.Lines.Add('*** ERRORE il path non esiste: '+ tPath);
                        inc(errCount);
                        continue;
                    end;
                    FileHTML.SaveToFile( fToSave );

                    lviewProdotti.Items.BeginUpdate;
                    try
                      ListItem:= lviewProdotti.Items.item[i];
                      //ListItem.Caption := 'OI010009';
    ...
    mmLog.Lines.Add(DatetimeToStr(now)+'   *** FINE Elaborazione Codici ***');
end;
...
begin
    ...
    FileAttrs := 0;
    //FileAttrs := faAnyFile;
    mmLog.Lines.Add('Search in ->  '+ aPathAndFileName);
    try
        ...
        mmLog.Lines.Add('Found '+ count.toString +' files.');
    end;
end;
...
//      strIdx := AnsiIndexText (tRequestPID, FileHTML );        // oppure IndexStr()
//      strIdx := strFileHTML.IndexOf(strToSearch);
  ...
  try
    ...
    for row := 1 to rowCount do begin
        ...
        *)
        ...
        if cell.IsString then begin
            arrCodici[row] := cell.ToString;
            ListItem.Caption := cell.ToString;
