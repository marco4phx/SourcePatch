...
begin
    ...
    // "OnCreate" code here:
    LogStep(10,10,10,10,  self.ClassName +' - onCreate.');
    LogStep(11,11,11,11,  self.ClassName +' version: '+ kFrameVersion);
    ...
    try
        ...
        on E: Exception do begin
            LogStep(12,12,12,12,  'EXCEPTION in JobList Create >> '+ E.ClassName +#10#13#9+ ' description: '+E.Message);
            raise;
        end;
    end;
...
begin
    LogStep(13,13,13,13, self.ClassName +' - OnDestroy.');
...
begin
    ...
    LogStep(14,14,14,14, self.ClassName +' - onAfterConstruction.');
...
begin
    LogStep(15,15,15,15, self.ClassName +' - onBeforeDestruction');
...
begin
    inherited;
    //Showmessage('Loaded');
    LogStep(16,16,16,16, 'frameFasi  - onLoaded.');
end;
*)
...
begin
    ...
    LogStep(17,17,17,17,  'Generato dati fittizi per demo Report.')
end;
...
begin
    if CAN_GetStatus( CHcan ) then
		...
		LogStep(18,18,18,18, 'PCAN STATUS on channel '+ intToHex(CHcan.Handle, 2) +'h  -> OK') //PCAN_ERROR_OK.
	else
    	// An error occurred.  We show the error.
        LogStep(19,19,19,19, 'PCAN STATUS on channel '+ intToHex(CHcan.Handle, 2) +'h  -> ERROR: '+ lastCAN_ErrMsg);
end;
...
begin
    if CAN_Reset( CHcan ) then
    	// The channel was successfully reset
		LogStep(20,20,20,20, 'PCAN RESET on channel '+ intToHex(CHcan.Handle, 2) +'h  -> OK') //PCAN_ERROR_OK.
	else
    	// An error occurred.  We show the error.
        LogStep(21,21,21,21, 'PCAN RESET on channel '+ intToHex(CHcan.Handle, 2) +'h  -> ERROR: '+ lastCAN_ErrMsg);
end;
...
begin
    if CAN_Release( CHcan ) then
		...
		LogStep(22,22,22,22, 'CLOSE channel '+ intToHex(CHcan.Handle, 2) +'h  -> OK') //PCAN_ERROR_OK.
	else
    	// An error occurred.  We show the error.
        LogStep(23,23,23,23, 'CLOSE channel '+ intToHex(CHcan.Handle, 2) +'h  -> ERROR: '+ lastCAN_ErrMsg);
end;
...
begin
    if CAN_Init( CHcan ) then
    	// The frame was successfully sent
		LogStep(24,24,24,24, 'INIT PCAN channel '+ intToHex(CHcan.Handle, 2) +'h  -> OK!') //PCAN_ERROR_OK.
	else
    	// An error occurred.  We show the error.
        LogStep(25,25,25,25, 'INIT PCAN channel '+ intToHex(CHcan.Handle, 2) +'h  -> ERROR: '+ lastCAN_ErrMsg);
end;
...
begin
    ...
    if CAN_SendFrame(Can) then
        // The frame was successfully sent
        LogStep(26,26,26,26, 'CAN Frame SENT Successfully,  ID:'+Can.CobID +'   LenPL:'+ intToStr(Can.LenPayLoad) +'   Frame:'+ hexLarge(Can.Frame))
    else
        LogStep(27,27,27,27,  lastCAN_ErrMsg );
end;
...
begin
    ...
    if CAN_ReceiveFrame( Can ) then begin
        ...
        LogStep(28,28,28,28, 'CAN Frame RECEIVED Successfully,  ID:'+Can.CobID +'   LenPL:'+intToStr(Can.LenPayLoad) +'   Frame:'+hexLarge(Can.Frame));
        ...
        LogStep(29,29,29,29,  lastCAN_ErrMsg );
end;
...
begin
    if CAN_SendNMT( CHcan, cmdStopNode, edtCanID.text ) then begin
        ...
        LogStep(30,30,30,30, 'CAN StopNode SENT Successfully to NodeID:'+ edtCanID.text );
        // quindi pulisco rx buffer.
        CAN_FlushRxBuffer(CHcan);
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(31,31,31,31, 'CAN StopNode ERROR  on NodeID:'+ edtCanID.text );
        LogStep(32,32,32,32,  lastCAN_ErrMsg );
    end;
end;
...
var
  i, nodes: integer;
begin
    LogStep(33,33,33,33, 'SCAN for active Node IDs...');
    if not CAN_FlushRxBuffer(CHcan) then begin
        LogStep(34,34,34,34,  lastCAN_ErrMsg );
        exit;
    end;
    ...
    if CAN_EnumNodes(CHcan, nodes) then begin  // Reset ALL nodes and wait for IDs
        ...
        if nodes > 0 then begin
            LogStep(35,35,35,35, 'Found '+ CanNodes.count.ToString +' CAN Nodes on PCan interface:'+CHcan.Handle.ToHexString);
            for i := 0 to CanNodes.count-1 do
                LogStep(36,36,36,36, #9#9'Node > '+ CanNodes.Strings[i] +'h');
        end
        else
            LogStep(37,37,37,37, 'NO CAN Nodes Found on PCan interface:'+CHcan.Handle.ToHexString);
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(38,38,38,38, 'ENUM CAN Nodes ERROR: '+ lastCAN_ErrMsg );
    end;
end;
...
begin
    try
        ...
        if CAN_NodeSaveAll( devAddr ) then begin
            // The frame was successfully sent
            LogStep(39,39,39,39, 'CAN SaveParams command SENT Successfully to NodeID:'+ devAddr );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            LogStep(40,40,40,40, 'CAN SaveParams ERROR on NodeID:'+ devAddr );
            LogStep(41,41,41,41,  lastCAN_ErrMsg );
            exit;
        end;
        ...
        if CAN_SendNMT( CHdut, cmdResetNode, devAddr ) then begin
            ...
            LogStep(42,42,42,42, 'CAN ResetNode SENT Successfully to NodeID:'+ devAddr );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            LogStep(43,43,43,43, 'CAN ResetNode ERROR  on NodeID:'+ devAddr );
            LogStep(44,44,44,44,  lastCAN_ErrMsg );
        end;
        // DUT reset & boot completed !
        // Read back cfg, non serve se WR senza errori...
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(45,45,45,45,  lastCAN_ErrMsg );
            exit;
        end;
        LogStep(46,46,46,46, 'OK FLUSH Can queue Successful.');    // preparato rx queue clean.
    except
      on E: Exception do begin
          LogStep(47,47,47,47,  'EXCEPTION on SAVE & Reset node '+devAddr+' >> '+ E.ClassName +#10#13#9+ ' description: '+E.Message);
      end;
    end;
end;
...
begin
    ...
    try
        ...
        if not CAN_WriteParam( Can ) then begin
            LogStep(48,48,48,48, 'CAN Stop HeartBeat ERROR on NodeID:'+ devAddr );
            LogStep(49,49,49,49, lastCAN_ErrMsg);
            exit;
        end;
        // The parameter was Written successfully.
        LogStep(50,50,50,50, 'CAN Stop HeartBeat command SENT Successfully to NodeID:'+ devAddr );

        if CAN_NodeRestoreDef( devAddr ) then begin
            // The frame was successfully sent
            LogStep(51,51,51,51, 'CAN RestoreParams command SENT Successfully to NodeID:'+ devAddr );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            LogStep(52,52,52,52, 'CAN RestoreParams ERROR on NodeID:'+ devAddr );
            LogStep(53,53,53,53,  lastCAN_ErrMsg );
            exit;
        end;
      ...
      on E: Exception do begin
          LogStep(54,54,54,54,  'EXCEPTION on RELOAD & Reset node '+devAddr+' >> '+ E.ClassName +#10#13#9+ ' description: '+E.Message);
      end;
    end;
end;
...
begin
    if CAN_FlushRxBuffer(CHcan) then begin
        LogStep(55,55,55,55, 'FLUSH PCAN Successful.');
    end
end;
...
begin
    if CAN_UnlockParams( edtCanID.text ) then begin
        // The frame was successfully sent
        LogStep(56,56,56,56, 'CAN Unlock command CONFIRMED by NodeID:'+ edtCanID.text );
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(57,57,57,57, 'CAN Unlock ERROR on NodeID:'+ edtCanID.text );
        LogStep(58,58,58,58,  lastCAN_ErrMsg );
    end;
end;
...
begin
    if CAN_LockParams( edtCanID.text ) then begin
        // The frame was successfully sent
        LogStep(59,59,59,59, 'CAN Lock command CONFIRMED by NodeID:'+ edtCanID.text );
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(60,60,60,60, 'CAN Lock ERROR on NodeID:'+ edtCanID.text );
        LogStep(61,61,61,61,  lastCAN_ErrMsg );
    end;
end;
...
begin
    if CAN_UnlockSN( edtCanID.text ) then begin
        // The frame was successfully sent
        LogStep(62,62,62,62, 'CAN UnlockSN command CONFIRMED by NodeID:'+ edtCanID.text );
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(63,63,63,63, 'CAN UnlockSN ERROR on NodeID:'+ edtCanID.text );
        LogStep(64,64,64,64,  lastCAN_ErrMsg );
    end;
end;
...
begin
    ...
    if CAN_SendNMT( CHcan, cmdResetNode, edtCanID.text ) then begin
        // The Reset was successfully sent
        LogStep(65,65,65,65, 'CAN ResetNode SENT Successfully to NodeID:'+ edtCanID.text );
        ...
        // l' Errore è bloccante, quindi esco.
        LogStep(66,66,66,66, 'CAN ResetNode ERROR  on NodeID:'+ edtCanID.text );
        LogStep(67,67,67,67,  lastCAN_ErrMsg );
    end;
    // DUT reset & boot completed !
end;
...
begin
    if CAN_SendNMT( CHcan, cmdStartNode, edtCanID.text ) then begin
        // The Start was successfully sent
        LogStep(68,68,68,68, 'CAN StartNode SENT Successfully to NodeID:'+ edtCanID.text );
        // quindi pulisco rx buffer.
        CAN_FlushRxBuffer(CHcan);
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(69,69,69,69, 'CAN StartNode ERROR  on NodeID:'+ edtCanID.text );
        LogStep(70,70,70,70,  lastCAN_ErrMsg );
    end;
end;
...
begin
    ...
    if CAN_WriteRawFrame(CHcan, edtCobID.text, edtRawCanData.text, txLen) then begin
		...
		LogStep(71,71,71,71, '> CAN Frame SENT Successfully,  COB:'+edtCobID.text +'   Len:'+edtRawCanLen.text+'   Data:'+edtRawCanData.text); //PCAN_ERROR_OK.
    end
	else
    	// An error occurred.  We show the error.
        LogStep(72,72,72,72,  '* '+ lastCAN_ErrMsg );
end;
...
begin
    ...
    if CAN_ReadRawFrame(CHcan, rcvdCob, RcvdLen, RcvdData) then begin
    	// The frame was successfully sent
        LogStep(73,73,73,73, '< CAN Frame RECEIVED Successfully,  COB:'+rcvdCob +'   Len:'+RcvdLen +'   Data:'+RcvdData);
        edtCobIDrcvd.text := rcvdCob;
        edtRawCanLenRcvd.text := rcvdLen;
        edtRawCanDataRcvd.text := rcvdData;
    end
	else
        LogStep(74,74,74,74,  '* '+ lastCAN_ErrMsg );
end;
...
begin
    ...
    if CAN_ReadRawFrame(CHref, rcvdCob, RcvdLen, RcvdData) then
    	// The frame was successfully sent
        LogStep(75,75,75,75, '<< RECEIVED CAN Frame,  COB:'+rcvdCob +'   Len:'+RcvdLen +'   Data:'+RcvdData);
	else
        LogStep(76,76,76,76,  '* ERROR onTimer Receive CAN: '+ lastCAN_ErrMsg );
end;
*)
...
begin
    ...
    LogStep(77,77,77,77, 'Xls Keywords - FrameVersion: '+ xls.DocumentProperties.GetStandardProperty(TPropertyId.Keywords));    // Keywords = F1.01
    ...
    LogStep(78,78,78,78, 'Xls Category - Frame4Devices: '+ tStr);
          ...
          try
            ...
            //ListItem.ImageIndex := -1;                    // per ora niente icon
              ...
              if not dicDataType.TryGetValue(CanCmd.DataType, aLen) then begin
                  ...
                  LogStep(79,79,79,79, 'Errore: CAN DataType NOT Found "'+ cell.ToString +'" in dictionary DataType.');
              end;
            ...
            on E: Exception do begin
              LogStep(80,80,80,80,  'EXCEPTION in Load PCAN Cmd from XLS >> '+ E.ClassName +#10#13#9+ ' description: '+E.Message);
...
begin
    ...
    try
      ...
      LogStep(81,81,81,81, 'Ordinamento file Parametri secondo WR Priority...');
      ...
      for row := from to rowCount do begin              // Skip 1st from-1 lines.
          ...
          try
              ...
              if cell.IsEmpty then begin                  // se manca, considero errore...
                  LogStep(82,82,82,82, 'Errore: CAN command Index NOT Found for Alias "'+ alias +'"');
                  inc(err);
                  exit;
              end;
              ...
              if not dicCanCmds.TryGetValue( alias, CanCmd ) then begin
                  LogStep(83,83,83,83, 'Errore: CAN command Alias NOT Found "'+ alias +'" in dictionary CanCmds.');
                  inc(err);
                  exit;
              end;
            ...
            //cfgListItem.ImageIndex := -1;                 // qui niente icon
              ...
              if not dicDataType.TryGetValue(cell.ToString, aLen) then begin
                  LogStep(84,84,84,84, 'Errore: CAN DataType NOT Found "'+ cell.ToString +'" in dictionary DataType.');
                  inc(err);
                  exit;
              end;
            ...
            //cfgListItem.ImageIndex := 20;                 // aggiorna icon su prima colonna.
              ...
              if (alias = _Operational_mode_) and AnsiContainsText(DutList[currDutIdx].ModoMisura, 'full') then begin
                  LogStep(85,85,85,85, 'WARNING: Operational-Mode override to Default 2 Axis for DUT:'+ edtDUTaddr.text );
            ...
            on E: Exception do begin
              LogStep(86,86,86,86,  'EXCEPTION in Load CFG CmdSet from XLS >> '+ E.ClassName +#10#13#9+ ' description: '+E.Message);
...
var
  ...
  tBeg, tEnd: TDateTime;
begin
    LogStep(87,87,87,87, 'Calcolo WorkTime '+ begFase +' e '+ endFase);
    ...
    if i >= 0 then begin
        ...
        LogStep(88,88,88,88, 'ERR in calcolo WorkTime, '+ begFase +' non trovata!');
...
begin
    ...
    try (*
        ...
        *)
        ...
        try
            LogStep(89,89,89,89, 'Aggiornamento dati Sessione EOL per S/N: '+ Sessione.Serial+'  su DB Produzione.' );
            ...
            with dmBase.tabSessionDut do begin
                ...
                LogStep(90,90,90,90, 'Locate session Record...');
                CheckBrowseMode;
                if Locate('CODICEPRODOTTO;LOTTO;SERIAL;RECMODE', VarArrayOf([Sessione.CodiceProdotto, Sessione.Lotto, Sessione.Serial, Sessione.RecMode]), []) then begin
                    ...
                    *)
                    Edit;
                    LogStep(91,91,91,91, 'Edit master tabSession...');
                    //
            ...
            LogStep(92,92,92,92, 'OK dati di Sessione aggiornati.');
            result := True;
        except
          on E: Exception do begin
...
begin
    ...
    if CAN_SendFrame(Can) then
        // The frame was successfully sent
        LogStep(93,93,93,93, 'CAN Frame SENT Successfully,  ID:'+Can.CobID +'   LenPL:'+ intToStr(Can.LenPayLoad) +'   Frame:'+ hexLarge(Can.Frame))
    else
        LogStep(94,94,94,94,  lastCAN_ErrMsg );
end;
...
begin
    ...
    if not CAN_CleanCH(CHdut) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(95,95,95,95,  lastCAN_ErrMsg );
        exit;
    end;
    LogStep(96,96,96,96, sLineBreak+'STATUS PCAN_OK');
    ...
    if CAN_SendNMT( CHdut, cmdResetNode, edtDUTaddr.text ) then begin
        // The Reset was successfully sent
        LogStep(97,97,97,97, 'CAN ResetNode SENT Successfully to NodeID:'+ edtDUTaddr.text );
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(98,98,98,98, 'CAN ResetNode ERROR  on NodeID:'+ edtDUTaddr.text );
        LogStep(99,99,99,99,  lastCAN_ErrMsg );
        exit;
    end;
    // DUT reset & boot completed !
    if not CAN_FlushRxBuffer(CHdut) then begin
        LogStep(100,100,100,100,  lastCAN_ErrMsg );
        exit;
    end;
    LogStep(101,101,101,101, 'OK FLUSH Can queue Successful.');    // preparato rx queue clean.
    ...
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(102,102,102,102, 'ERROR on UNLOCK params for DUT:'+ edtDUTaddr.text );
        LogStep(103,103,103,103,  lastCAN_ErrMsg );
        exit;     // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(104,104,104,104, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    ...
    try
        ...
        for CalDeg := deg180 to deg270 do begin
            ...
            if not CAN_ReadParam( Can ) then begin         // Delay standard è 50 msec.
                LogStep(105,105,105,105, 'ERROR on READ param ['+CanCmd1+'] for DUT:'+ edtDUTaddr.text );
                LogStep(106,106,106,106,  lastCAN_ErrMsg );
                break;
            end;
            // The parameter was Read successfully.
            val1 := Can.Data;
            iValueReadX := StrToIntDef('$'+ val1, 0);      // $ perchè payload è stringa ma sempre Hex !
            LogStep(107,107,107,107, #9#9+ CanCmd1 +' = $'+ val1 );
            ...
            Can.NodeID   := edtDUTaddr.text;               // str ID del DUT.
            if not CAN_ReadParam( Can ) then begin         // Delay standard è 50 msec.
                LogStep(108,108,108,108, 'ERROR on READ param ['+CanCmd2+'] for DUT:'+ edtDUTaddr.text );
                LogStep(109,109,109,109,  lastCAN_ErrMsg );
                break;
            end;
            // The parameter was Read successfully.
            val2 := Can.Data;
            iValueReadY := StrToIntDef('$'+val2, 0);       // $ perchè payload è stringa ma sempre Hex !
            LogStep(110,110,110,110, #9#9+ CanCmd2 +' = $'+ val2 );
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            // l' Errore è bloccante, quindi esco.
            LogStep(111,111,111,111, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(112,112,112,112,  lastCAN_ErrMsg );
        end
        else
            LogStep(113,113,113,113, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    end;
end;
...
begin
    ...
    if not CAN_CleanCH(CHdut) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(114,114,114,114,  lastCAN_ErrMsg );
        exit;
    end;
    LogStep(115,115,115,115, 'STATUS PCAN_OK, check DUT Calibrato...');
    // The channel is Ready to work.

    // Reset DUT necessario per portare in RAM i parametri, nel caso siano appena stati calibrati (settati in eeprom)!
    if CAN_SendNMT( CHdut, cmdResetNode, edtDUTaddr.text ) then begin
        // The Reset was successfully sent
        LogStep(116,116,116,116, 'CAN ResetNode SENT Successfully to NodeID:'+ edtDUTaddr.text );
    end
    else begin
        // l' Errore è bloccante, quindi esco.
        LogStep(117,117,117,117, 'CAN ResetNode ERROR  on NodeID:'+ edtDUTaddr.text );
        LogStep(118,118,118,118,  lastCAN_ErrMsg );
        exit;
    end;
    // DUT reset & boot completed !
    if not CAN_FlushRxBuffer(CHdut) then begin
        LogStep(119,119,119,119,  lastCAN_ErrMsg );
        exit;
    end;
    LogStep(120,120,120,120, 'OK FLUSH Can queue Successful.');    // preparato rx queue clean.

    // Sblocca area protetta 5555h del DUT per precauzione, se non fosse già da setup fase.
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(121,121,121,121, 'ERROR on UNLOCK params for DUT:'+ edtDUTaddr.text );
        LogStep(122,122,122,122,  lastCAN_ErrMsg );
        exit;     // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(123,123,123,123, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    ...
    try
        for CalDeg := deg180 to deg270 do begin
            ...
            Can.NodeID   := edtDUTaddr.text;               // str ID del DUT.
            if not CAN_ReadParam( Can ) then begin         // Delay standard è 50 msec.
                LogStep(124,124,124,124, 'ERROR on READ param ['+CanCmd1+'] for DUT:'+ edtDUTaddr.text );
                LogStep(125,125,125,125,  lastCAN_ErrMsg );
                break;
            end;
            // The parameter was Read successfully.
            val1 := Can.Data;
            ...
            Can.NodeID   := edtDUTaddr.text;               // str ID del DUT.
            if not CAN_ReadParam( Can ) then begin         // Delay standard è 50 msec.
                LogStep(126,126,126,126, 'ERROR on READ param ['+CanCmd2+'] for DUT:'+ edtDUTaddr.text );
                LogStep(127,127,127,127,  lastCAN_ErrMsg );
                break;
            end;
            // The parameter was Read successfully.
            val2 := Can.Data;
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            // l' Errore è bloccante, quindi esco.
            LogStep(128,128,128,128, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(129,129,129,129,  lastCAN_ErrMsg );
        end
        else
            LogStep(130,130,130,130, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    end;
end;
...
begin
    ...
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        ...
        LogStep(131,131,131,131, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            ...
            LogStep(132,132,132,132, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(133,133,133,133, 'ALL nodes Reset Successfully');
    LogStep(134,134,134,134, 'FLUSH queue All CanCH...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(135,135,135,135,  'ERROR on Flush CHref : '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(136,136,136,136,  'ERROR on Flush CHdut: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(137,137,137,137, 'OK FLUSH All CanCH Successful.');    // preparato rx queue clean.
    ...
    if not CAN_GetResolution( edtDUTaddr.text, DUT_resolution) then begin // Delay standard è 25 msec
        LogStep(138,138,138,138,  lastCAN_ErrMsg );
        exit;
    end;
    // The Device Resolution [1,10,100,1000] was Read successfully !
    LogStep(139,139,139,139, 'OK DUT Parameter '+Can.Index+'-'+Can.Subidx+'h READ Successfully from NodeID:'+ edtDutAddr.text );
    LogStep(140,140,140,140, #9#9'DUT '+ Can.Command +' = '+ DUT_resolution.ToString );
    ...
    if not CAN_WriteParam( Can ) then begin
        LogStep(141,141,141,141, lastCAN_ErrMsg);
        exit;
    end;
    // The parameter was Written successfully.
    LogStep(142,142,142,142, 'OK DUT Parameter '+Can.Index+'-'+Can.Subidx+'h STORED Successfully to NodeID:'+ edtDUTaddr.text );
    LogStep(143,143,143,143, #9#9'DUT '+ Can.Command +' = '+ kTimer4cyclicalTxd );

    // DUT START node, enter operational state
    if not CAN_SendNMT( CHdut, cmdStartNode, edtDUTaddr.text ) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(144,144,144,144, 'ERROR on StartNode DUT:'+ edtDUTaddr.text );
        LogStep(145,145,145,145,  lastCAN_ErrMsg );
        exit;
    end;
    // The Start was successfully sent
    LogStep(146,146,146,146, 'OK StartNode SENT Successfully to DUT: '+ edtDUTaddr.text );
...
begin
    ...
    tmrCanRead.enabled := False;
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        LogStep(147,147,147,147, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            LogStep(148,148,148,148, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(149,149,149,149, 'ALL nodes Reset Successfully');
    LogStep(150,150,150,150, 'FLUSH queue All CanCH...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(151,151,151,151,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(152,152,152,152,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(153,153,153,153, 'OK FLUSH CanDUT queue Successful.');    // preparato rx queue clean.
    ...
    if not CAN_GetResolution( edtRef1Addr.text, Ref1_resolution) then begin // Delay standard è 25 msec
        LogStep(154,154,154,154,  lastCAN_ErrMsg );
        exit;
    end;
    // The Device Resolution [1,10,100,1000] was Read successfully !
    LogStep(155,155,155,155, 'OK Ref1_resolution READ Successfully from NodeID:'+ edtRef1Addr.text );
    LogStep(156,156,156,156, #9#9'Ref1 Resolution = '+ Ref1_resolution.ToString );
    ...
    if not CAN_WriteParam( Can ) then begin
        LogStep(157,157,157,157, lastCAN_ErrMsg);
        exit;
    end;
    // The parameter was Written successfully.
    LogStep(158,158,158,158, 'OK REF Parameter '+Can.Index+'-'+Can.Subidx+'h STORED Successfully to NodeID:'+ edtRef1Addr.text );
    LogStep(159,159,159,159, #9#9'REF1 '+ Can.Command +' = '+ kTimer4cyclicalTxd );

    // REF START node, enter operational state
    if not CAN_SendNMT( CHref, cmdStartNode, edtRef1Addr.text ) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(160,160,160,160, 'ERROR on StartNode REF:'+ edtRef1Addr.text );
        LogStep(161,161,161,161,  lastCAN_ErrMsg );
        exit;
    end;
    // The Start was successfully sent
    LogStep(162,162,162,162, 'OK StartNode SENT Successfully to REF: '+ edtRef1Addr.text );
...
begin
    ...
    tmrCanRead.enabled := False;
    LogStep(163,163,163,163, 'RESET REF e DUT...');
    // Poi Reset ALL nodes, casomai fossero già in Start mode.
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        LogStep(164,164,164,164, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            LogStep(165,165,165,165, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(166,166,166,166, 'ALL nodes Reset Successfully');
    LogStep(167,167,167,167, 'FLUSH queue All CanCH...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(168,168,168,168,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(169,169,169,169,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(170,170,170,170, 'OK FLUSH CanDUT queue Successful.');    // preparato rx queue clean.
    ...
    if not CAN_GetResolution( edtRef2Addr.text, Ref2_resolution) then begin // Delay standard è 25 msec
        LogStep(171,171,171,171,  lastCAN_ErrMsg );
        exit;
    end;
    // The Device Resolution [1,10,100,1000] was Read successfully !
    LogStep(172,172,172,172, 'OK REF2 Parameter '+Can.Index+'-'+Can.Subidx+'h READ Successfully from NodeID:'+ edtRef2Addr.text );
    LogStep(173,173,173,173, #9#9'REF2 '+ Can.Command +' = '+ Ref2_resolution.ToString );
    ...
    if not CAN_WriteParam( Can ) then begin
        LogStep(174,174,174,174, lastCAN_ErrMsg);
        exit;
    end;
    // The parameter was Written successfully.
    LogStep(175,175,175,175, 'OK REF2 Parameter '+Can.Index+'-'+Can.Subidx+'h STORED Successfully to NodeID:'+ edtRef2Addr.text );
    LogStep(176,176,176,176, #9#9'REF2 '+ Can.Command +' = '+ kTimer4cyclicalTxd );

    // REF START node, enter operational state
    if not CAN_SendNMT( CHref, cmdStartNode, edtRef2Addr.text ) then begin
        // l' Errore è bloccante, quindi esco.
        LogStep(177,177,177,177, 'ERROR on StartNode REF:'+ edtRef2Addr.text );
        LogStep(178,178,178,178,  lastCAN_ErrMsg );
        exit;
    end;
    // The Start was successfully sent
    LogStep(179,179,179,179, 'OK StartNode SENT Successfully to REF2: '+ edtRef2Addr.text );
    CAN_FlushRxBuffer(CHref);                     // prepara rx queue clean
...
begin
    ...
    // Poi Reset ALL nodes, casomai fossero già in Start mode.
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        LogStep(180,180,180,180, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            LogStep(181,181,181,181, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(182,182,182,182, 'ALL nodes Reset Successfully');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(183,183,183,183,  lastCAN_ErrMsg );
        exit;
    end;
    LogStep(184,184,184,184, 'OK FLUSH CanREF queue Successful.');    // preparato rx queue clean.
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(185,185,185,185,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(186,186,186,186, 'OK FLUSH CanDUT queue Successful.');    // preparato rx queue clean.
    ...
    LogStep(187,187,187,187, 'OK Dut read STOPPED: '+ edtDUTaddr.text );
end;
...
begin
    ...
    // Poi Reset ALL nodes, casomai fossero già in Start mode.
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        LogStep(188,188,188,188, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            LogStep(189,189,189,189, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(190,190,190,190, 'ALL nodes Reset Successfully');
    LogStep(191,191,191,191, 'FLUSH queue All CanCH...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(192,192,192,192,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(193,193,193,193,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(194,194,194,194, 'OK FLUSH All CanCH queue Successful.');    // preparato rx queue clean.
    ...
    LogStep(195,195,195,195, 'OK Ref1 reads from node: '+ edtRef1Addr.text +' STOPPED.');
end;
...
begin
    ...
    // Poi Reset ALL nodes, casomai fossero già in Start mode.
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        LogStep(196,196,196,196, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            LogStep(197,197,197,197, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(198,198,198,198, 'ALL nodes Reset Successfully');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(199,199,199,199,  lastCAN_ErrMsg );
        exit;
    end;
    LogStep(200,200,200,200, 'OK FLUSH CanREF queue Successful.');
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(201,201,201,201,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(202,202,202,202, 'OK FLUSH CanDUT queue Successful.');    // preparato rx queue clean.
    ...
    LogStep(203,203,203,203, 'OK Ref2 reads from node: '+ edtRef2Addr.text +' STOPPED.');
end;
...
begin
    // serve leggere DUT_resolution per calcolo DEGs quando acquisisco samples via can read.
    if not CAN_GetResolution( edtDUTaddr.text, DUT_resolution) then begin // Delay standard è 25 msec
        LogStep(204,204,204,204,  lastCAN_ErrMsg );
        exit;
    end;
    // The Device Resolution [1,10,100,1000] was Read successfully !
    LogStep(205,205,205,205, #9#9'DUT resolution = '+ DUT_resolution.ToString );

    // legge Angolo X da DUT as INT16. (6010-0)
    if not CAN_Read_ANGLE( edtDUTaddr.text, 'X', iDeg) then begin // Delay standard è 25 msec
        LogStep(206,206,206,206,  lastCAN_ErrMsg );
        exit;
    end;
    ...
    if not CAN_Read_ANGLE( edtDUTaddr.text, 'Y', iDeg) then begin // Delay standard è 25 msec
        LogStep(207,207,207,207,  lastCAN_ErrMsg );
        exit;
    end;
...
begin
    ...
    if SetupFase_A(Params) then
        LogStep(208,208,208,208,  'Fase setup SUCCESS.')
    else
        LogStep(209,209,209,209,  'Fase FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if not dmBase.FindFile( anMp3 ) then
        ...
        try
          ...
          //mPlayer.Close;            //
            LogStep(210,210,210,210, 'Play memo audio: '+ anMp3);
          //mPlayer.FileName := OpenDialog.FileName;
            ...
            LogStep(211,211,211,211, 'Tracks: '+ mPlayer.Tracks.ToString);                   // sempre 1 sola.
            LogStep(212,212,212,212, 'TrackLength: '+ mPlayer.TrackLength[1].ToString);      // integer variabile
            LogStep(213,213,213,213, 'Position: '+ mPlayer.Position.ToString);               // corrente, da 0..TrackLength[1]
            mPlayer.Play;
...
begin
    ...
    if SetupFase_C(Params) then
        LogStep(214,214,214,214,  'Fase setup SUCCESS.')
    else
        LogStep(215,215,215,215,  'Fase FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_C8(Params) then
        LogStep(216,216,216,216,  'Step SUCCESS.')
    else
        LogStep(217,217,217,217,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_C9(Params) then
        LogStep(218,218,218,218,  'Step SUCCESS.')
    else
        LogStep(219,219,219,219,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_C10(Params) then
        LogStep(220,220,220,220,  'Step SUCCESS.')
    else
        LogStep(221,221,221,221,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_C11(Params) then
        LogStep(222,222,222,222,  'Step SUCCESS.')
    else
        LogStep(223,223,223,223,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if SetupFase_D(Params) then
        LogStep(224,224,224,224,  'Fase setup SUCCESS.')
    else
        LogStep(225,225,225,225,  'Fase FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if SetupFase_E(Params) then
        LogStep(226,226,226,226,  'Fase setup SUCCESS.')
    else
        LogStep(227,227,227,227,  'Fase FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_D12(Params) then
        LogStep(228,228,228,228,  'Step SUCCESS.')
    else
        LogStep(229,229,229,229,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_D13(Params) then
        LogStep(230,230,230,230,  'Step SUCCESS.')
    else
        LogStep(231,231,231,231,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_D14(Params) then
        LogStep(232,232,232,232,  'Step SUCCESS.')
    else
        LogStep(233,233,233,233,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_D15(Params) then
        LogStep(234,234,234,234,  'Step SUCCESS.')
    else
        LogStep(235,235,235,235,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_D16(Params) then
        LogStep(236,236,236,236,  'Step SUCCESS.')
    else
        LogStep(237,237,237,237,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E17(Params) then
        LogStep(238,238,238,238,  'Step SUCCESS.')
    else
        LogStep(239,239,239,239,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E18(Params) then
        LogStep(240,240,240,240,  'Step SUCCESS.')
    else
        LogStep(241,241,241,241,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E19(Params) then
        LogStep(242,242,242,242,  'Step SUCCESS.')
    else
        LogStep(243,243,243,243,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E20(Params) then
        LogStep(244,244,244,244,  'Step SUCCESS.')
    else
        LogStep(245,245,245,245,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E21(Params) then
        LogStep(246,246,246,246,  'Step SUCCESS.')
    else
        LogStep(247,247,247,247,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E22(Params) then
        LogStep(248,248,248,248,  'Step SUCCESS.')
    else
        LogStep(249,249,249,249,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_E23(Params) then
        LogStep(250,250,250,250,  'Step SUCCESS.')
    else
        LogStep(251,251,251,251,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if SetupFase_F(Params) then
        LogStep(252,252,252,252,  'Fase setup SUCCESS.')
    else
        LogStep(253,253,253,253,  'Fase FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_F25(Params) then
        LogStep(254,254,254,254,  'Step SUCCESS.')
    else
        LogStep(255,255,255,255,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_F26(Params) then
        LogStep(256,256,256,256,  'Step SUCCESS.')
    else
        LogStep(257,257,257,257,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_F27(Params) then
        LogStep(258,258,258,258,  'Step SUCCESS.')
    else
        LogStep(259,259,259,259,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_F28(Params) then
        LogStep(260,260,260,260,  'Step SUCCESS.')
    else
        LogStep(261,261,261,261,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if SetupFase_G(Params) then
        LogStep(262,262,262,262,  'Fase setup SUCCESS.')
    else
        LogStep(263,263,263,263,  'Fase FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_G29(Params) then
        LogStep(264,264,264,264,  'Step SUCCESS.')
    else
        LogStep(265,265,265,265,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_G30(Params) then
        LogStep(266,266,266,266,  'Step SUCCESS.')
    else
        LogStep(267,267,267,267,  'Step FAILED: '+ Params.retMsg );
end;
...
begin
    ...
    if RunStep_G31(Params) then
        LogStep(268,268,268,268,  'Step SUCCESS.')
    else
        LogStep(269,269,269,269,  'Step FAILED: '+ Params.retMsg );
end;
...
{   come setup di fase E serve solo un reset device per preparare R/W parameters
  ...
  //mainForm.mmLog.lines.add('run SetupFase_...');
    ...
    LogStep(270,270,270,270, 'OK Setup Fase DUMMY.' );
...
begin
    ...
    LogStep(271,271,271,271, 'OK RunStep DUMMY.' );
...
begin
  ...
  //mainForm.mmLog.lines.add('run SetupFase_A...');
    ...
    // init Com Channels
//  LogStep(272,272,272,272, 'Inizializzazione Comm Channels...');
//  Scan4ComInterfaces();

    // init CAN Channels
    LogStep(273,273,273,273, 'Inizializzazione CAN Channels...');
    CAN_Release(CHref);                     //
    ...
    if not Scan4PCanInterfaces() then begin    // Cerca Canali PCAN disponibili
        ...
        LogStep(274,274,274,274, JobParams.retMsg);
        exit;
    end;
    ...
    for i := 0 to CanChannels.count -1 do begin
        ...
        if addr = '51h' then begin
            LogStep(275,275,275,275, #9#9+ Addr +'  >> PCAN per i Reference');
            cboxCanREF.ItemIndex := i;
            cboxCanREF.Style.color := clLime;
        end
        else if addr = '52h' then begin
            LogStep(276,276,276,276, #9#9+ Addr +'  >> PCAN per il Device in Test');
    ...
    if CanChannels.count = 1 then begin
        ...
        cboxCanDUT.Style.color := clLime;
        LogStep(279,279,279,279, 'WARNING - PCAN bus '+ CanChannels.Strings[0] +' è Condiviso fra REF e DUT !');
        // Override Handle
        ...
        LogStep(280,280,280,280, 'INFO - PCAN bus Channels dedicati per REF e DUT.');
        // Override Handle
    ...
    LogStep(281,281,281,281, 'Verifica se Report EOL già esistente...');
    //rptFile := vReportPath +'\'+ Sessione.CodiceProdotto +'_'+ Sessione.Lotto +'\'+ Sessione.CodiceProdotto +'_EOL_'+ Sessione.Lotto + Sessione.Serial +'.xls';
    kXls := '.xls';
    ...
    if DirectoryExists( vReportPath ) then begin     // Server disponibile e report di produzione
        ...
        LogStep(282,282,282,282, #9+ eolReport);
        ...
        if dmBase.FindFile( eolReport ) then begin
            ...
            LogStep(283,283,283,283, 'OK report Non trovato.');
    end
    else begin
        ...
        JobParams.retCode := resERROR;
        LogStep(284,284,284,284, JobParams.retMsg);
    ...
    if not CAN_Init(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(285,285,285,285, JobParams.retMsg);
        exit;
    end;
  	// CAN Ref interface is Ready
    LogStep(286,286,286,286, 'OK REF PCAN Init.');
  //if numCanCH > 1 then          // CHref = CHdut
    if CHdut.Handle <> CHref.Handle then
        if not CAN_Init(CHdut) then begin
            JobParams.retMsg := 'DUT PCAN_ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(287,287,287,287, JobParams.retMsg);
            exit;
        end;
  	// CAN Dut interface is Ready
    LogStep(288,288,288,288, 'PCAN Init OK'); //PCAN_ERROR_OK.
    ...
    LogStep(289,289,289,289, 'Loading CMD file: '+ cmdFile);
    if not Load_PCAN_CMDs_fromXLS( cmdFile ) then begin
        JobParams.retMsg := 'ERROR on Load CAN Command list: '+ cmdFile;
        JobParams.retCode := resERROR;
        LogStep(290,290,290,290, JobParams.retMsg);
        exit;
    end;
    LogStep(291,291,291,291, 'OK PCAN Commands loaded.');
    ...
    LogStep(292,292,292,292, 'RESET ALL nodes...');
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL Ref nodes!
        JobParams.retMsg := 'ERROR on Reset CHref Nodes: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(293,293,293,293, lastCAN_ErrMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, '00h' ) then begin  // Reset ALL Dut nodes!
            ...
            JobParams.retCode := resERROR;
            LogStep(294,294,294,294, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(295,295,295,295, 'ALL nodes Reset Successfully');
    dmBase.wait(500);                         // sembra necessario, forse il delay in CAN_SendNMT() non basta...!
    LogStep(296,296,296,296, 'FLUSH All CAN queue...');
    if not CAN_FlushRxBuffer(CHref) then begin
        ...
        LogStep(297,297,297,297,  JobParams.retMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            ...
            LogStep(298,298,298,298,  JobParams.retMsg );
            exit;
        end;
    LogStep(299,299,299,299, 'ALL CAN queue Flushed Successfully.');    // preparato rx queue clean.
    ...
    LogStep(300,300,300,300, 'SCAN for active Node IDs...');
    ...
    if not CAN_EnumNodes(CHref, nodes) then begin        // Reset ALL nodes and wait for IDs
        ...
        JobParams.retCode := resERROR;
        LogStep(301,301,301,301, JobParams.retMsg);
        exit;
    end;
    LogStep(302,302,302,302, #9'Found '+ nodes.ToString +' Nodes on PCAN '+ CHref.Handle.ToHexString);
    if CHdut.Handle <> CHref.Handle then begin
        if not CAN_EnumNodes(CHdut, nodes) then begin        // Reset ALL nodes and wait for IDs
            ...
            JobParams.retCode := resERROR;
            LogStep(303,303,303,303, JobParams.retMsg);
            exit;
        end;
        LogStep(304,304,304,304, #9'Found '+ nodes.ToString +' Nodes on PCAN '+ CHdut.Handle.ToHexString);
    end;
    if CanNodes.count = 0 then begin
        JobParams.retMsg := 'NO CAN Nodes Found !';
        JobParams.retCode := resERROR;
        LogStep(305,305,305,305, JobParams.retMsg);
        exit;
    end;
    LogStep(306,306,306,306, 'Total of '+ CanNodes.count.ToString +' CAN Nodes on '+numCanCH.toString+' PCan interface.');
    ...
    *)
    for i := 0 to CanNodes.count-1 do begin
        ...
        if addr = '7E' then begin
            ...
            LogStep(310,310,310,310, #9#9+ Addr +'h  >> node DUT ?');
            edtDutAddr.Text  := addr +'h';
            edtDutAddr.Style.color := clLime;
            edtCanID.Text  := addr +'h';
        end;
    ...
    if (edtRef2Addr.Text = '') or (edtDutAddr.Text = '') then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(311,311,311,311, JobParams.retMsg);
        exit;
    end;
...
begin
    ...
    try
        ...
        if not CAN_GetResolution( edtRef1Addr.text, Ref1_resolution) then begin  // Delay read è 25 msec
            ...
            LogStep(312,312,312,312,  lastCAN_ErrMsg );
            exit;
        end;
        // The Device Resolution [1,10,100,1000] was Read successfully !
        // NB: il valore Letto in Ref1_resolution è uguale alla risoluzione, 100 = Centesimi di°
        LogStep(313,313,313,313, 'OK Ref1 '+ _Angular_Resolution_ +' READ Successfully from NodeID:'+ edtRef1Addr.text );
        LogStep(314,314,314,314, #9#9'REF1 '+ _Angular_Resolution_ +' = '+ Ref1_resolution.ToString );
        edtResolREF1.text := Ref1_resolution.ToString;
        ...
        if not CAN_GetResolution( edtRef2Addr.text, Ref2_resolution) then begin  // Delay read è 25 msec
            ...
            LogStep(315,315,315,315,  lastCAN_ErrMsg );
            exit;
        end;
        // The Device Resolution [1,10,100,1000] was Read successfully !
        // NB: il valore Letto in Ref2_resolution è uguale alla risoluzione, 100 = Centesimi di°
        LogStep(316,316,316,316, 'OK Ref2 '+ _Angular_Resolution_ +' READ Successfully from NodeID:'+ edtRef2Addr.text );
        LogStep(317,317,317,317, #9#9'REF2 '+ _Angular_Resolution_ +' = '+ Ref2_resolution.ToString );
        edtResolREF2.text := Ref2_resolution.ToString;

        // serve leggere DUT_resolution per calcolo DEGs quando acquisisco samples via can read.
        if not CAN_GetResolution( edtDUTaddr.text, DUT_resolution) then begin // Delay standard è 25 msec
            lastCAN_ErrMsg := 'DUT Read '+ _Angular_Resolution_ +' ERROR: '+ lastCAN_ErrMsg;
            LogStep(318,318,318,318,  lastCAN_ErrMsg );
            exit;
        end;
        // The Device Resolution [1,10,100,1000] was Read successfully !
        // NB: il valore Letto in DUT_resolution è uguale alla risoluzione, 100 = Centesimi di°
        LogStep(319,319,319,319, 'OK DUT '+ _Angular_Resolution_ +' READ Successfully from NodeID:'+ edtDutAddr.text );
        LogStep(320,320,320,320, #9#9'DUT '+ _Angular_Resolution_ +' = '+ DUT_resolution.ToString );
        edtResolDUT.text := DUT_resolution.ToString;
        ...
        if REF1_resolution <> 100 then begin
            LogStep(321,321,321,321, 'WARNING: Ref1 angular Resolution <> 1/100 [centesimi]  >>  will Override.' );
            if not CAN_SetResolution( edtRef1addr.text, degCentesimi) then begin // Delay standard è 25 msec
                ...
                LogStep(322,322,322,322,  lastCAN_ErrMsg );
                exit;
            end;
            // The Device Resolution [1,10,{100},1000] was Written successfully ! (NB: valori invertiti rispetto a ciò che va in subidx!)
            REF1_resolution   := 100;
            edtResolREF1.text := REF1_resolution.ToString;
            LogStep(323,323,323,323, 'OK new REF1 '+ _Angular_Resolution_ +' STORED Successfully to NodeID:'+ edtRef1addr.text );
            LogStep(324,324,324,324, #9#9'REF1 '+ _Angular_Resolution_ +' = '+ REF1_resolution.ToString );
        end;
        if REF2_resolution <> 100 then begin
            LogStep(325,325,325,325, 'WARNING: Ref2 angular Resolution <> 1/100 [centesimi]  >>  will Override.' );
            if not CAN_SetResolution( edtRef2addr.text, degCentesimi) then begin // Delay standard è 25 msec
                ...
                LogStep(326,326,326,326,  lastCAN_ErrMsg );
                exit;
            end;
            // The Device Resolution [1,10,{100},1000] was Written successfully ! (NB: valori invertiti rispetto a ciò che va in subidx!)
            REF2_resolution   := 100;
            edtResolREF2.text := REF2_resolution.ToString;
            LogStep(327,327,327,327, 'OK new REF2 '+ _Angular_Resolution_ +' STORED Successfully to NodeID:'+ edtRef2addr.text );
            LogStep(328,328,328,328, #9#9'REF2 '+ _Angular_Resolution_ +' = '+ REF2_resolution.ToString );
        end;
        if DUT_resolution <> 100 then begin
            LogStep(329,329,329,329, 'WARNING: Dut angular Resolution <> 1/100 [centesimi]  >>  will Override.' );
            if not CAN_SetResolution( edtDUTaddr.text, degCentesimi) then begin // Delay standard è 25 msec
                ...
                LogStep(330,330,330,330,  lastCAN_ErrMsg );
                exit;
            end;
            // The Device Resolution [1,10,{100},1000] was Written successfully ! (NB: valori invertiti rispetto a ciò che va in subidx!)
            DUT_resolution   := 100;
            edtResolDUT.text := DUT_resolution.ToString;
            LogStep(331,331,331,331, 'OK new DUT '+ _Angular_Resolution_ +' STORED Successfully to NodeID:'+ edtDutAddr.text );
            LogStep(332,332,332,332, #9#9'DUT '+ _Angular_Resolution_ +' = '+ DUT_resolution.ToString );
            ...
            if CAN_NodeSaveAll( edtDUTaddr.text ) then begin
                // Ok, niente msec. attesa perchè WR in eeprom già completato.
                LogStep(333,333,333,333, 'CAN SaveParams command SENT Successfully to DUT:'+ edtDUTaddr.text );
            end
            else begin
                // l' Errore è bloccante, quindi esco.
                lastCAN_ErrMsg := 'CAN SaveParams ERROR on DUT:'+ edtDUTaddr.text +' - ERROR: '+ lastCAN_ErrMsg;
                LogStep(334,334,334,334,  lastCAN_ErrMsg );
                exit;
            end;
          ...
          //if not CAN_SendNMT( cmdResetNode, '00h' ) then begin           // Reset ALL nodes!
                ...
                LogStep(335,335,335,335,  lastCAN_ErrMsg );
                exit;
            end;
            // The Reset was successfully sent
            LogStep(336,336,336,336, 'CAN ResetNode SENT Successfully to NodeID:'+ edtDUTaddr.text );
            // DUT reset & boot completed !
            if not CAN_FlushRxBuffer(CHdut) then begin
                lastCAN_ErrMsg := 'ERROR on Flush CAN buffer: '+ lastCAN_ErrMsg;
                LogStep(337,337,337,337,  lastCAN_ErrMsg );
                exit;
            end;
            LogStep(338,338,338,338, 'OK FLUSH Can queue Successful.');
            // lascio Rx Can queue clean.
        end;
        result := True;
    except
      on E: Exception do begin
          LogStep(339,339,339,339,  'EXCEPTION on Force Resolution to [degCentesimi] >> '+ E.ClassName +#10#13#9+ ' description: '+E.Message);
      end;
    end;
end;
...
{   come setup di fase B serve controllare status PCAN, presenza dei Reference device
    ...
    if not CAN_GetStatus(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(340,340,340,340, JobParams.retMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_GetStatus(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(341,341,341,341, JobParams.retMsg);
            exit;
        end;
  	// The channels are Ready
    LogStep(342,342,342,342, 'Fase_B: PCAN STATUS OK');
    ...
    LogStep(343,343,343,343, 'RESET REF2 e DUT...');
  //if not CAN_SendNMT( cmdResetNode, '00h' ) then begin  // Reset ALL nodes!
    if not CAN_SendNMT( CHref, cmdResetNode, edtRef2Addr.text ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(344,344,344,344, lastCAN_ErrMsg);
        exit;
    end;
    // The Reset was successfully sent
    if not CAN_SendNMT( CHdut, cmdResetNode, edtDutAddr.text ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(345,345,345,345, lastCAN_ErrMsg);
        exit;
    end;
    LogStep(346,346,346,346, 'ALL Nodes Reset Successfully');
    LogStep(347,347,347,347, 'FLUSH queue All Can CHANNELS...');
    // clean boot frames.
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(348,348,348,348,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(349,349,349,349,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(350,350,350,350, 'OK FLUSH All CanCH Done.');  // preparato rx queue clean.
    ...
    if i <> 2 then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(351,351,351,351, JobParams.retMsg);
        exit;
    end;
    // qui il tipo Ref trovato a questo indirizzo è corretto per i successivi usi previsti !!
    LogStep(352,352,352,352, 'OK REF type '+ i.toString+'Axes present on Address: '+ edtRef2Addr.text); //PCAN_ERROR_OK.
    ...
    if not CAN_CheckNodeID(edtDutAddr.text) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(353,353,353,353, JobParams.retMsg);
        exit;
    end;
    LogStep(354,354,354,354, 'OK DUT present on Address: '+ edtDutAddr.text); //PCAN_ERROR_OK.
    ...
    if not CAN_WriteParam( Can ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(355,355,355,355, JobParams.retMsg);
        exit;
    end;
    // The parameter was Written successfully.
    LogStep(356,356,356,356, 'OK Ref2 '+ _Timer_for_cyclical_txd_ +' STORED Successfully to NodeID:'+ edtRef2Addr.text );
    LogStep(357,357,357,357, #9#9'Ref2 '+ _Timer_for_cyclical_txd_ +' = '+ kTimer4cyclicalTxd );
    CalibRecords.Clear;          // Clear removes all keys and values from the dictionary. The Count property is set to 0.
    ...
    LogStep(358,358,358,358, 'OK, completato Setup per Fase B1.' );
...
begin
{
    ...
    if not CAN_GetStatus(CHref) then begin
        ...
        LogStep(359,359,359,359, JobParams.retMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_GetStatus(CHdut) then begin
            ...
            LogStep(360,360,360,360, JobParams.retMsg);
            exit;
        end;
  	// The channels are Ready
    LogStep(361,361,361,361, 'Fase_C: STATUS PCAN_OK');
    ...
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(362,362,362,362, lastCAN_ErrMsg);
        exit;
    end;
    LogStep(363,363,363,363, 'REF2 ResetNode SENT Successfully to NodeID:'+ edtRef2Addr.text );
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, edtDutAddr.text ) then begin
            JobParams.retMsg := 'DUT ResetNode '+edtDutAddr.text+' ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(364,364,364,364, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(365,365,365,365, 'ALL Nodes Reset Successful');
    LogStep(366,366,366,366, 'FLUSH queue All Can CHANNELS...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(367,367,367,367,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(368,368,368,368,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(369,369,369,369, 'OK FLUSH All CanCH Successful.');    // preparato rx queue clean.
    ...
    if i <> 1 then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(370,370,370,370, JobParams.retMsg);
        exit;
    end;
    // qui sarebbe da valutare se il tipo Ref trovato a questo indirizzo è corretto per i successivi Test previsti !!
    LogStep(371,371,371,371, 'OK REF type '+ i.toString+'Axes present on Address: '+ edtRef1Addr.text); //PCAN_ERROR_OK.

    // Verifica presenza DUT.
    if not CAN_CheckNodeID(edtDutAddr.text) then begin
        JobParams.retMsg := 'DUT check NodeID ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(372,372,372,372, JobParams.retMsg);
        exit;
    end;
    LogStep(373,373,373,373, 'OK DUT present on Address: '+ edtDutAddr.text); //PCAN_ERROR_OK.
    ...
    if not CAN_WriteParam( Can ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(374,374,374,374, JobParams.retMsg);
        exit;
    end;
    // The parameter was Written successfully.
    LogStep(375,375,375,375, 'OK Ref1 '+ _Timer_for_cyclical_txd_ +' STORED Successfully to NodeID:'+ edtRef1Addr.text );
    LogStep(376,376,376,376, #9#9'Ref1 '+ _Timer_for_cyclical_txd_ +' = '+ kTimer4cyclicalTxd );
    ...
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(377,377,377,377,  JobParams.retMsg );
        exit; // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(378,378,378,378, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    try
        ...
        LogStep(379,379,379,379, #9#9'Set DUT param '+ Can.Command +' = 0000h');
        LogStep(380,380,380,380, 'OK, param ERR1 Reset Successfully on DutID:'+ edtDUTaddr.text );
        ...
        // The parameter was Written successfully.
        LogStep(381,381,381,381, #9#9'Set DUT param '+ Can.Command +' = 0000h');
        LogStep(382,382,382,382, 'OK, param ERR2 Reset Successfully on DutID:'+ edtDUTaddr.text );
        ...
        if CAN_NodeSaveAll( edtDUTaddr.text ) then begin
            // The frame was successfully sent
            LogStep(383,383,383,383, 'CAN SaveParams command SENT Successfully to NodeID:'+ edtDUTaddr.text );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            JobParams.retMsg := 'DUT SaveParams ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(384,384,384,384, 'ERROR on Save Params of DUT:'+ edtDUTaddr.text );
            LogStep(385,385,385,385,  lastCAN_ErrMsg );
            exit;
        end;
        // Ok, niente msec. attesa perchè WR in eeprom già completato.
        ...
        if CAN_SendNMT( CHdut, cmdResetNode, edtDUTaddr.text ) then begin
            // The Reset was successfully sent
            LogStep(386,386,386,386, 'CAN ResetNode SENT Successfully to NodeID:'+ edtDUTaddr.text );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            JobParams.retMsg := 'DUT ResetNode ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(387,387,387,387, 'ERROR on ResetNode DUT:'+ edtDUTaddr.text );
            LogStep(388,388,388,388,  JobParams.retMsg );
            exit;
        end;
        // DUT reset & boot completed !
        // Read back cfg, non serve se WR senza errori...
        if not CAN_FlushRxBuffer(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(389,389,389,389,  JobParams.retMsg );
            exit;
        end;
        LogStep(390,390,390,390, 'OK FLUSH CHdut queue Successful.');    // preparato rx queue clean.

        CalibRecords.Clear;          // Clear removes all keys and values from the dictionary. The Count property is set to 0.
        TestXRecords.Clear;          // reset sortDictionary con records dati dei test, anche se qui non usati.
        CarattXRecords.Clear;        // comunque se rieseguo la calibrazione, i dati di caratterizzazione non sono più validi !

        LogStep(391,391,391,391, 'OK, completato Setup per Fase-C' );
        JobParams.retMsg := _Completato_;
        JobParams.retCode := resOK;
        result := True;
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(392,392,392,392, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(393,393,393,393,  lastCAN_ErrMsg );
        end
        else
            LogStep(394,394,394,394, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    end;
end;
...
{   come setup di fasi D e E serve controllare status PCAN, presenza del Reference e DUT devices,
  ...
  //mainForm.mmLog.lines.add('run SetupFasi_DE...');
    ...
    if not CAN_GetStatus(CHref) then begin
        ...
        LogStep(395,395,395,395, JobParams.retMsg);
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_GetStatus(CHdut) then begin
            ...
            LogStep(396,396,396,396, JobParams.retMsg);
            exit;
        end;
  	// The channels are Ready
    LogStep(397,397,397,397, 'Fase_D&E: STATUS PCAN_OK');
  ...
  //img1Seq1.Left := 552;
    ...
    LogStep(398,398,398,398, 'RESET REF1 e DUT...');
    if not CAN_SendNMT( CHref, cmdResetNode, edtRef1Addr.text ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(399,399,399,399, lastCAN_ErrMsg);
        exit;
    end;
    // The Reset was successfully sent
    if not CAN_SendNMT( CHdut, cmdResetNode, edtDutAddr.text ) then begin
        JobParams.retMsg := 'DUT ResetNode '+edtDutAddr.text+' ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(400,400,400,400, lastCAN_ErrMsg);
        exit;
    end;
    LogStep(401,401,401,401, 'ALL Nodes Reset Successfully');
    LogStep(402,402,402,402, 'FLUSH queue All Can CHANNELS...');
    // clean boot frames.
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(403,403,403,403,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(404,404,404,404,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(405,405,405,405, 'OK FLUSH All CanCH Done.');  // preparato rx queue clean.
    ...
    if i <> 1 then begin
        JobParams.retMsg := 'ERROR Unexpected REF1 device Type.';
        JobParams.retCode := resERROR;
        LogStep(406,406,406,406, JobParams.retMsg);
        exit;
    end;
    // qui sarebbe da valutare se il tipo Ref trovato a questo indirizzo è corretto per i successivi Test previsti !!
    LogStep(407,407,407,407, 'OK REF type'+ i.toString+'Axis present on Address: '+ edtRef1Addr.text); //PCAN_ERROR_OK.

    // Verifica presenza DUT.
    if not CAN_CheckNodeID(edtDutAddr.text) then begin
        JobParams.retMsg := 'REF1 ID ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(408,408,408,408, JobParams.retMsg);
        exit;
    end;
    LogStep(409,409,409,409, 'OK DUT present on Address: '+ edtDutAddr.text); //PCAN_ERROR_OK.
    ...
    if not CAN_WriteParam( Can ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(410,410,410,410, JobParams.retMsg);
        exit;
    end;
    // The parameter was Written successfully.
    LogStep(411,411,411,411, 'OK Ref1 '+ Can.Command +' STORED Successfully to NodeID:'+ edtRef1Addr.text );
    LogStep(412,412,412,412, #9#9'Ref1 '+ Can.Command +' = '+ kTimer4cyclicalTxd +' msec');
    ...
    if DutList[currDutIdx].ErrOneAxis = 0 then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(413,413,413,413, JobParams.retMsg);
        exit;
    end;
    LogStep(414,414,414,414, 'Valore MAX Error per device: '+ DutList[currDutIdx].CodArticolo );
    edtErrMax1Asse.text  := format('%6.2f°', [DutList[currDutIdx].ErrOneAxis]);
    LogStep(415,415,415,415,  #9#9'ErrMax 1Asse = '+  edtErrMax1Asse.text );
    ...
    LogStep(416,416,416,416, 'OK, completato Setup per Fasi-DE' );
...
{   come setup di fase F serve solo un reset device per preparare R/W parameters
  ...
  //mainForm.mmLog.lines.add('run SetupFase_F...');
    ...
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(417,417,417,417, JobParams.retMsg);
        exit;
    end;
    // The Reset was successfully sent
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, edtDutAddr.text ) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(418,418,418,418, JobParams.retMsg);
            exit;
        end;
    LogStep(419,419,419,419, 'ALL Nodes Reset Successfully');
    // clean boot frames.
    dmBase.wait(300);       // anche solo per evitare click multipli involontari da parte user.
    LogStep(420,420,420,420, 'FLUSH queue All Can CHANNELS...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(421,421,421,421,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(422,422,422,422,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(423,423,423,423, 'OK FLUSH All CanCH Successful.');    // preparato rx queue clean.
    dmBase.wait(200);     // anche solo per evitare click multipli involontari da parte user.

    LogStep(424,424,424,424, 'OK, completato Setup per Fase E' );
    JobParams.retMsg := _Completato_;
    JobParams.retCode := resOK;
    result := True;
end;
...
begin
{
    ...
    LogStep(425,425,425,425, 'Attempting connection to Optoi POSTGRES DB on '+ dmBase.pgConn.Server);
    ...
    if dmBase.pgConn.Connected then begin
        ...
        LogStep(426,426,426,426, #9'Connesso al Server '+ dmBase.pgConn.Server +':'+ dmBase.pgConn.Database);
        LogStep(427,427,427,427, #9'Versione: '+ dmBase.pgConn.ServerVersionFull);  // o ServerVersion
    end
    else begin
        LogSysEvent(svERROR, 2320, 'DB Connection open FAILED !');
        JobParams.retMsg := 'ERROR on Connecting DB > '+ err;
        JobParams.retCode := resERROR;
        LogStep(428,428,428,428, JobParams.retMsg);
        exit;
    end;
    ...
    try
       ...
       LogStep(429,429,429,429, 'Opened table: '+dmBase.tabSessionDut.TableName);
    except
    ...
    try
       ...
       LogStep(430,430,430,430, 'Opened table: '+dmBase.tabTestData.TableName);
    except
    ...
    try
       ...
       LogStep(431,431,431,431, 'Opened table: '+dmBase.tabDutParams.TableName);
    except
    ...
    if res then begin
        LogStep(432,432,432,432, 'DB tables schema Produzione READY.');
    end
    else begin
    ...
    LogStep(433,433,433,433, 'OK, completato Setup per Fase F.' );
    dmBase.wait(1000);
    JobParams.retMsg := _Completato_;
    JobParams.retCode := resOK;
    result := True;
...
var
  errCount: integer;
begin
    LogStep(434,434,434,434, JobList.Strings[currJob]+' - '+ TJobRecord( JobList.Objects[ currJob ]).Description);
    result := False;
    JobParams.retCode := resUNDEFINED;
    JobParams.retMsg := _Incompleto_;
    ...
    try
        ...
        try
          ...
          if errCount > 0 then begin
              ...
              JobParams.retCode := resOK;
              LogStep(435,435,435,435, 'OK acquisito campo Note.');
              result := True;
          end;
        ...
        LogStep(436,436,436,436, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
    end;
end;
...
begin
    ...
    LogStep(437,437,437,437, 'OK, completata ultima Fase H' );
    dmBase.wait(700);
...
begin
  ...
  //mainForm.mmLog.lines.add('RunStep_1...');
    ...
    LogStep(438,438,438,438, 'Loading CFG file '+ cfgFile);
    if not Load_CFG_CmdSet_fromXLS( cfgFile ) then begin
        JobParams.retMsg := 'Step1 ERROR on Load CFG Command list: '+ cfgFile;
        JobParams.retCode := resERROR;
        LogStep(439,439,439,439, JobParams.retMsg);
        exit;
    end;
    LogStep(440,440,440,440, 'OK CFG Command list loaded.');
    JobParams.retMsg := _Completato_;
    JobParams.retCode := resOK;
    result := True;
end;
...
begin
  //mainForm.mmLog.lines.add('RunStep_2...');
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(441,441,441,441,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(442,442,442,442,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(443,443,443,443, JobList.Strings[currJob]+': STATUS PCAN_OK');
    ...
    LogStep(444,444,444,444, 'OK REF node Stopped.'); //PCAN_ERROR_OK.
    *)
    ...
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        ...
        JobParams.retMsg := 'DUT UnlockParams ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(445,445,445,445,  JobParams.retMsg );
        exit(False);       // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(446,446,446,446, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    ...
    try
          ...
          for i := 0 to lviewCFG.Items.Count -1 do begin   // Scorre Parametri CAN
              ...
              if lviewCFG.Items[i].Checked  then begin          // processo solo i checked, anche se ho travasato in lviewCFG solo i default,
                  ...
                  if not dicCanCmds.TryGetValue( cmdAlias, CanCmd) then begin
                      ...
                      JobParams.retCode := resERROR;
                      LogStep(447,447,447,447,  JobParams.retMsg );
                      exit;
                  end;
                  ...
                  if CAN_SendFrame(Can) then begin
                      ...
                      LogStep(448,448,448,448, 'CAN Frame SENT Successfully,  ID:'+Can.CobID +'   LenPL:'+ intToStr(Can.LenPayLoad) +'   Frame:'+ hexLarge(Can.Frame));
                  end
                  else begin
                      // singolo Errore non è bloccante, lascio portare a termine loop su tutti i params.
                      LogStep(449,449,449,449,  lastCAN_ErrMsg );
                      inc(errCount);
                      continue;
                  end;
                  ...
                  if CAN_ReceiveFrame( Can ) then begin
                      // OK, the frame was successfully read.
                      // Received Payload in Can.Data
                      LogStep(450,450,450,450, 'CAN Frame RECEIVED Successfully,  ID:'+Can.CobID +'   LenPL:'+intToStr(Can.LenPayLoad) +'   Frame:'+hexLarge(Can.Frame));
                      ...
                      // singolo Errore non è bloccante, lascio portare a termine loop su tutti i params.
                      LogStep(451,451,451,451,  lastCAN_ErrMsg );
                      inc(errCount);
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(453,453,453,453, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(454,454,454,454,  lastCAN_ErrMsg );
            result := False;
        end
        else
            LogStep(455,455,455,455, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );

        LogStep(456,456,456,456, #9'Found '+ ToWrite.ToString +' Parameter to write');
        LogStep(457,457,457,457, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
        if ToWrite > 0 then
...
begin
  //mainForm.mmLog.lines.add('RunStep_3...');
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(458,458,458,458,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(459,459,459,459,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(460,460,460,460, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !
    ...
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        ...
        JobParams.retMsg := 'DUT UnlockParams ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(461,461,461,461,  JobParams.retMsg );
        exit(False);       // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(462,462,462,462, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
  ...
  //cbxSelectAllClick(self);         // ovvero trovati diversi dal previsto.
    ...
    try
          ...
          for i := 0 to lviewCFG.Items.Count -1 do begin   // Scorre Parametri CAN
              ...
              if lviewCFG.Items[i].Checked  then begin           // Write solo su quelli trovati, e marcati, come diversi da default.
                  ...
                  if not dicCanCmds.TryGetValue( cmdAlias, CanCmd) then begin
                      JobParams.retMsg := 'ERROR Alias NOT Found "'+ cmdAlias +'" in dictionary CanCmds.';
                      JobParams.retCode := resERROR;
                      LogStep(463,463,463,463,  JobParams.retMsg );
                      exit;
                  end;
                ...
                //if ListItem.SubItems[8] = 'SNUM' then begin
                      ...
                      if not CAN_UnlockSN( edtDUTaddr.text ) then begin
                          ...
                          JobParams.retCode := resERROR;
                          LogStep(464,464,464,464, 'ERROR on UnlockSN for DUT:'+ edtDUTaddr.text );
                          LogStep(465,465,465,465,  JobParams.retMsg );
                          inc(errCount);
                          exit;           // unlock non riuscito, quindi non serve lock finally !
                      end;
                      // Non serve e non esiste un comando di LockSN, ma solo alla fine per intera area 5555h.
                      LogStep(466,466,466,466, 'OK, DUT UnlockSN CONFIRMED by NodeID:'+ edtDUTaddr.text );
                      ...
                      if dicCanCmds.TryGetValue( _SN_Value_2wr_, CanCmd) then begin
                          ...
                          JobParams.retMsg := 'ERROR Alias NOT Found "'+ cmdAlias +'" in dictionary CanCmds.';
                          JobParams.retCode := resERROR;
                          LogStep(467,467,467,467,  JobParams.retMsg );
                          exit;
                      end;
                  end;
                  ...
                  if CAN_SendFrame(Can) then begin
                      // The frame was successfully sent
                      // la funzione Write accetta Can.Data con spazi
                      LogStep(468,468,468,468, 'CAN Parameter SENT Successfully,  ID:'+Can.CobID +'   LenPL:'+ intToStr(Can.LenPayLoad) +'   Frame:'+ hexLarge(Can.Frame));
                  end
                  else begin
                      // singolo Errore non è bloccante, lascio portare a termine loop su tutti i params.
                      LogStep(469,469,469,469,  lastCAN_ErrMsg );
                      inc(errCount);
                      continue;
                  end;
                  ...
                  if CAN_ReceiveFrame( Can ) then begin
                      ...
                      inc(countWritten);
                      // Received Payload in Can.Data
                      LogStep(470,470,470,470, 'CAN Parameter STORED Successfully,  ID:'+Can.CobID +'   LenPL:'+intToStr(Can.LenPayLoad) +'   Frame:'+hexLarge(Can.Frame));
                    ...
                    //ListItem.ImageIndex := 6;                    // Status OK su prima colonna.
                      ...
                      if cmdAlias = _Node_ID_ then begin
                          if edtDUTaddr.text <> tData then begin
                              LogStep(471,471,471,471, #9#9'Warning CAN Node_ID change from:'+edtDUTaddr.text +'  To:'+ tData);
                      ...
                      if cmdAlias = _Baud_Rate_ then begin  // se il Baud_Rate non era default, devo aggiornare TEdit con nuovo rate,
                          if cbxCANbaud.text <> tData then begin
                              LogStep(472,472,472,472, #9#9+'Warning CAN Baud_Rate change from:'+cbxCANbaud.text +'  To:'+ tData);
                    ...
                    //ListItem.ImageIndex := 2;                  // Status Error su prima colonna.
                      LogStep(473,473,473,473,  lastCAN_ErrMsg );
                      inc(errCount);
                      continue;
                  end;
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(474,474,474,474, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(475,475,475,475,  lastCAN_ErrMsg );
            result := False;             // necessario override result!
        end
        else
            LogStep(476,476,476,476, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
        ...
        LogStep(477,477,477,477,  #9'Parameters written: '+ intToStr(countWritten));
        LogStep(478,478,478,478, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
...
begin
  //mainForm.mmLog.lines.add('RunStep_4...');
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(479,479,479,479,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(480,480,480,480,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(481,481,481,481, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !
    ...
    try
        try
            ...
            if CAN_NodeSaveAll( edtDUTaddr.text ) then begin
                // The frame was successfully sent
                LogStep(482,482,482,482, 'CAN SaveParams command SENT Successfully to NodeID:'+ edtDUTaddr.text );
            end
            else begin
                // l' Errore è bloccante, quindi esco.
                LogStep(483,483,483,483, 'CAN SaveParams ERROR on NodeID:'+ edtDUTaddr.text );
                LogStep(484,484,484,484,  lastCAN_ErrMsg );
                inc(errCount);
                exit;
            end;
            ...
            if CAN_SendNMT( CHdut, cmdResetNode, edtDUTaddr.text ) then begin
                // The Reset was successfully sent
                LogStep(485,485,485,485, 'CAN ResetNode SENT Successfully to NodeID:'+ edtDUTaddr.text );
            end
            else begin
                // l' Errore è bloccante, quindi esco.
                LogStep(486,486,486,486, 'CAN ResetNode ERROR  on NodeID:'+ edtDUTaddr.text );
                LogStep(487,487,487,487,  lastCAN_ErrMsg );
                inc(errCount);
                exit;
            end;
            // DUT reset & boot completed !
        ...
        LogStep(488,488,488,488, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
        dmBase.wait(500);// anche solo per evitare click multipli involontari da parte user.
    end;
end;
...
begin
{
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(489,489,489,489,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(490,490,490,490,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(491,491,491,491, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !
    ...
    try
        ...
        if not CAN_SendNMT( CHref, cmdStartNode, edtRef2Addr.text ) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(492,492,492,492, 'ERROR on StartNode REF2:'+ edtRef2Addr.text );
            LogStep(493,493,493,493,  lastCAN_ErrMsg );
            inc(errCount);
            exit;
        end;
        // The Start was successfully sent
        LogStep(494,494,494,494, 'OK StartNode SENT Successfully to Ref2: '+ edtRef2Addr.text );
        ...
        try
          while true do begin // resta in endless loop fino a regolazione ok, o user abort.
              ...
              //LogStep(495,495,495,495, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) );
              TInterlocked.BitTestAndClear(SampleReady, 2);     // Reset SampleReady bit 2 = from Ref 2 assi
              //LogStep(496,496,496,496, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) +' =>> RELEASED');
              ...
              LogStep(497,497,497,497, 'REF_in_Range counters   X='+ xREFinRange_counter.toString +'  Y='+ yREFinRange_counter.toString );
              ...
              if not CAN_SendNMT( CHref, cmdStopNode, edtRef2Addr.text ) then begin
                  ...
                  JobParams.retCode := resERROR;
                  LogStep(498,498,498,498,  JobParams.retMsg );
                  exit;
              end;
              LogStep(499,499,499,499, 'OK REF2 node Stopped.');
              if not CAN_FlushRxBuffer(CHref) then begin
                  JobParams.retMsg := 'ERROR on Flush CHref buffer: '+ lastCAN_ErrMsg;
                  JobParams.retCode := resERROR;
                  LogStep(500,500,500,500,  JobParams.retMsg );
                  exit;
              end;
              LogStep(501,501,501,501, 'OK FLUSH Can queue Successful.');    // preparato rx queue clean.
        ...
        LogStep(502,502,502,502,  JobList.Strings[currJob] +' ERRORS = '+ intToStr(errCount));
...
begin
{
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(503,503,503,503,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(504,504,504,504,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(505,505,505,505, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !
    ...
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        ...
        JobParams.retMsg := 'DUT UnlockParams ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(506,506,506,506,  JobParams.retMsg );
        inc(errCount);
        exit;           // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(507,507,507,507, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    ...
    try
        ...
        if not CAN_SendNMT( CHref, cmdStartNode, edtRef1Addr.text ) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(508,508,508,508, 'ERROR on StartNode REF1:'+ edtRef1Addr.text );
            LogStep(509,509,509,509,  lastCAN_ErrMsg );
            inc(errCount);
            exit;
        end;
        // The Start was successfully sent
        LogStep(510,510,510,510, 'OK StartNode SENT Successfully to Ref1: '+ edtRef1Addr.text );

        LogStep(511,511,511,511, 'FLUSH queue All Can CHANNELS...');
        if not CAN_FlushRxBuffer(CHref) then begin           // prepara all rxCan queue clean
            LogStep(512,512,512,512,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
            inc(errCount);
            exit;
        end;
        if CHdut.Handle <> CHref.Handle then
            if not CAN_FlushRxBuffer(CHdut) then begin
                LogStep(513,513,513,513,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
                inc(errCount);
                exit;
            end;
        LogStep(514,514,514,514, 'OK FLUSH All CanCH Successful.');    // preparato rx queue clean.
        ...
        try
          while true do begin // resta in endless loop fino a regolazione ok, o user abort.
              ...
              //LogStep(515,515,515,515, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) );
              TInterlocked.BitTestAndClear(SampleReady, 1);     // Reset SampleReady bit 1 = from Ref 1 assi
              //LogStep(516,516,516,516, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) +' =>> RELEASED');
              ...
              LogStep(517,517,517,517, 'REF_in_Range counters   X='+ xREFinRange_counter.toString +'  Y='+ yREFinRange_counter.toString );
              ...
              if not CAN_SendNMT( CHref, cmdStopNode, edtRef1Addr.text ) then begin
                  ...
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(518,518,518,518,  JobParams.retMsg );
                  exit;
              end;
              LogStep(519,519,519,519, 'OK REF1 node Stopped.');
              if not CAN_FlushRxBuffer(CHref) then begin
                  JobParams.retMsg := 'ERROR on Flush CHref buffer: '+ lastCAN_ErrMsg;
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(520,520,520,520,  JobParams.retMsg );
                  exit;
              end;
              LogStep(521,521,521,521, 'OK FLUSH CHref queue Successful.');    // preparato rx queue clean.
              ...
              for i := 1 to vNumSamples4Media do begin  // default vNumSamples4Media è 10.
                  ...
                  if not CAN_ReadParam( Can ) then begin         // Delay standard è 50 msec.
                      ...
                      JobParams.retCode := resERROR;
                      inc(errCount);
                      LogStep(522,522,522,522,  JobParams.retMsg );
                      break;
                  end;
                  // The parameter was Read successfully.
                  iValueReadX := StrToIntDef('$'+Can.Data, 0);       // $ perchè payload è stringa ma sempre Hex !
                  mediaX := mediaX + iValueReadX;
                  //LogStep(523,523,523,523, #9#9'Get DUT '+ Can.Command +' = '+ Can.Data );
                  //LogStep(524,524,524,524, 'OK, DUT Parameter ['+Can.Command+'] READ Successfully from DutID:'+ edtDUTaddr.text );
                  ...
                  if not CAN_ReadParam( Can ) then begin         // Delay standard è 50 msec.
                      ...
                      JobParams.retCode := resERROR;
                      inc(errCount);
                      LogStep(525,525,525,525,  JobParams.retMsg );
                      break;
                  end;
                  // The parameter was Read successfully.
                  iValueReadY := StrToIntDef('$'+Can.Data, 0);       // $ perchè payload è stringa ma sempre Hex !
                  mediaY := mediaY + iValueReadY;
                  //LogStep(526,526,526,526, #9#9'Get DUT '+ Can.Command +' = '+ Can.Data );
                  //LogStep(527,527,527,527, 'OK, DUT Parameter ['+Can.Command+'] READ Successfully from DutID:'+ edtDUTaddr.text );
                  LogStep(528,528,528,528, #9'Got DUT    X = '+ iValueReadX.toString +'     Y = '+ iValueReadY.toString );
            ...
            //mediaY := mediaY div 10;  tronca su quel minore, perdendo il peso degli altri 9 !
              ...
              LogStep(529,529,529,529, 'OK, DUT mediaX = '+edtDutX.text+'   mediaY = '+ edtDutY.text );
              ...
              if not CAN_WriteParam( Can ) then begin
                  ...
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(530,530,530,530,  JobParams.retMsg );
                  break;
              end;
              LogStep(531,531,531,531, #9#9'Set DUT param '+ Can.Command +' = '+ edtDutX.text +' ('+mediaX.ToHexString(4)+'h)');
              LogStep(532,532,532,532, 'OK, DUT 1Ax Calibration  '+Can.Index+'-'+Can.Subidx+'h STORED Successfully to DutID:'+ edtDUTaddr.text );
              ...
              if not CAN_WriteParam( Can ) then begin
                  ...
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(533,533,533,533,  JobParams.retMsg );
                  break;
              end;
              // The parameter was Written successfully.
              LogStep(534,534,534,534, #9#9'Set DUT param '+ Can.Command +' = '+ edtDutY.text +' ('+mediaY.ToHexString(4)+'h)');
              LogStep(535,535,535,535, 'OK, DUT 1Ax Calibration  '+Can.Index+'-'+Can.Subidx+'h STORED Successfully to DutID:'+ edtDUTaddr.text );
          ...
          if not dicCanCmds.TryGetValue( CanCmd1, CanCmd ) then begin
              ...
              JobParams.retCode := resERROR;
              inc(errCount);
              LogStep(536,536,536,536,  JobParams.retMsg );
              exit;
          end;
          ...
          if not dicCanCmds.TryGetValue( CanCmd2, CanCmd ) then begin
              ...
              JobParams.retCode := resERROR;
              inc(errCount);
              LogStep(537,537,537,537,  JobParams.retMsg );
              exit;
          end;
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            ...
            JobParams.retMsg := 'DUT LockParams ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(538,538,538,538, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(539,539,539,539,  lastCAN_ErrMsg );
            inc(errCount);
            result := False;
        end
        else
            LogStep(540,540,540,540, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );

      //TestRecord.free;  No, si perde il record di calibrazione appena memorizzato!  che serve in report !
        LogStep(541,541,541,541,  JobList.Strings[currJob] +' ERRORS = '+ intToStr(errCount));
...
var
  errCount: integer;
begin
    LogStep(542,542,542,542, JobList.Strings[currJob]+' - '+ TJobRecord( JobList.Objects[ currJob ]).Description);
    result := False;
    JobParams.retCode := resUNDEFINED;
    JobParams.retMsg := _Incompleto_;
    ...
    try
        ...
        try
          ...
          if errCount > 0 then begin
              ...
              JobParams.retMsg := _Completato_; // Ok, btn premuto senza altri errori.
              JobParams.retCode := resOK;
              LogStep(543,543,543,543, 'OK azzerato encoder Roll.');
              result := True;
          end;
        ...
        LogStep(544,544,544,544, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
    end;
end;
...
var
  ...
  Ch: string;
begin
    LogStep(545,545,545,545, 'SCAN for PCAN-USB Adapters...');   // carica la relativa ComBox CAN-ID in Admin tab.
    result := False;
    numCanCH := 0;
    ...
    if CAN_EnumChannels() then begin    // Cerca le interfacces USB PCan presenti
        ...
        if CanChannels.count > 0 then begin
            LogStep(546,546,546,546, 'Found '+ CanChannels.count.ToString +' PCAN interfaces:');
            // dopo display lista dettagliata, la riduco per uso nelle combobox
            for i := 0 to CanChannels.count-1 do begin
                LogStep(547,547,547,547, #9#9'Channel > '+ CanChannels.Strings[i] );
            ...
            LogStep(548,548,548,548, 'WARNING: '+ lastCAN_ErrMsg );
            exit;
        end;
        ...
        // l' Errore è bloccante, quindi esco.
        LogStep(549,549,549,549, 'ENUM PCAN interfaces ERROR: '+ lastCAN_ErrMsg );
    end;
end;
...
var
  errCount: integer;
begin
    LogStep(550,550,550,550, JobList.Strings[currJob]+' - '+ TJobRecord( JobList.Objects[ currJob ]).Description);
    result := False;
    JobParams.retCode := resUNDEFINED;
    JobParams.retMsg := _Incompleto_;
    ...
    try
        ...
        try
          ...
          if errCount > 0 then begin
              ...
              JobParams.retMsg := _Completato_; // Ok, btn premuto senza altri errori.
              JobParams.retCode := resOK;
              LogStep(551,551,551,551, 'OK posizionato Roll a 5.000');
              result := True;
          end;
        ...
        LogStep(552,552,552,552, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
    end;
end;
...
begin
{
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(553,553,553,553,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(554,554,554,554,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(555,555,555,555, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !
    LogStep(556,556,556,556, 'FLUSH queue All Can CHANNELS...');
    if not CAN_FlushRxBuffer(CHref) then begin    // prepara rx queue clean
        LogStep(557,557,557,557,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(558,558,558,558,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(559,559,559,559, 'OK FLUSH All CanCH Successful.');    // preparato rx queue clean.
    ...
    if not CAN_SendNMT( CHref, cmdStartNode, edtRef1Addr.text ) then begin
        ...
        JobParams.retMsg := 'REF1 StartNode ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(560,560,560,560, 'ERROR on StartNode REF1:'+ edtRef1Addr.text );
        LogStep(561,561,561,561,  lastCAN_ErrMsg );
        exit;
    end;
    // The Start was successfully sent
    LogStep(562,562,562,562, 'OK StartNode SENT Successfully to REF1: '+ edtRef1Addr.text );
    ...
    try
        ...
        try
          while true do begin // resta in endless loop fino a validazione ok, o user abort.
              ...
              //LogStep(563,563,563,563, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) );
              // Se resetto subito il flag sono sicuro di non perdere il prossimo set da parte del timer.
              TInterlocked.BitTestAndClear(SampleReady, 1);     // Reset SampleReady bit 1 = from Ref1 Asse  (SampleReady=2/0)
              //LogStep(564,564,564,564, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) +' =>> RELEASED');
              ...
              LogStep(565,565,565,565, 'REF_in_Range counters   X='+ xREFinRange_counter.toString );
              ...
              if not CAN_SendNMT( CHref, cmdStopNode, edtRef1Addr.text ) then begin
                  JobParams.retMsg := 'ERROR on StopNode REF1: '+ lastCAN_ErrMsg;
                  JobParams.retCode := resERROR;
                  LogStep(566,566,566,566,  lastCAN_ErrMsg );
                  inc(errCount);
                  exit;
              end;
              LogStep(567,567,567,567, 'OK REF node Stopped.');

              dmBase.Wait(25);   // lascia digerire i comandi ai nodes.
              LogStep(568,568,568,568, 'ALL Cyclic transmission stopped Successfully');
              // e svuota da eventuali residui
              if not CAN_FlushRxBuffer(CHref) then begin
                  JobParams.retMsg := 'ERROR on Flush CHref buffer: '+ lastCAN_ErrMsg;
                  JobParams.retCode := resERROR;
                  LogStep(569,569,569,569,  JobParams.retMsg );
                  inc(errCount);
                  exit;
              end;
              LogStep(570,570,570,570, 'OK FLUSH CHref queue Successful.');    // preparato rx queue clean.

              // OK, Ref è su Angolo richiesto, e Y ignorato, a 90°
              // con tolleranza ±0,04 deg.
              LogText(#9#9+ format('    REF_X1 = %6.2f   > Preliminare', [currXRef]));

              LogStep(571,571,571,571, 'Acquisizione campioni dal DUT per Caratterizzazione...');
              // leggo Almeno 10 valori DUT non troppo ravvicinati, a 50-100 msec...
              // Al momento il delay interno all CAN_ReadParam() è fisso a 50 msec !
              // Qui posso fare direttamente la Media dei valori senza aspettare DUT stabile, perchè
              // la tolleranza è molto stretta e quindi è implicita la scarsa fluttuazione dei valori !
              ...
              for i := 1 to vNumSamples4Media do begin  // default vNumSamples4Media è 10.
                  ...
                  if not CAN_Read_ANGLE( edtDUTaddr.text, 'X', iDeg) then begin // Delay standard è 25 msec
                      ...
                      JobParams.retCode := resERROR;
                      LogStep(572,572,572,572,  JobParams.retMsg );
                      inc(errCount);
                      break;
                  end;
                  // The currXDut was Read successfully !
            ...
            //                          tronca su quel minore, perdendo il peso degli altri 9 !
              ...
              LogStep(573,573,573,573, 'OK, Letture DUT mediaX = '+edtDutX.text );
//*
              ...
              if not CAN_Read_ANGLE( edtREF1addr.text, 'X', iDeg) then begin // Delay standard è 25 msec
                  ...
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(574,574,574,574,  JobParams.retMsg );
                  exit;
              end;
...
//
        ...
        tmrFasiBlink.Enabled := False;     // termina blink.
        img1Seq1.Visible := False;         //
        LogStep(575,575,575,575, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
...
begin
    ...
    JobParams.retCode := resUNDEFINED;
    JobParams.retMsg := _Incompleto_;
  //LogStep(576,576,576,576, JobList.Strings[currJob]+' - '+ TJobRecord( JobList.Objects[ currJob ]).Description);
...
(*
    ...
    if not CarattXRecords.ContainsKey( deg315 ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(577,577,577,577,  JobParams.retMsg );
        exit;
    end;
...
*)
    ...
    if not CarattXRecords.TryGetValue( deg315, carattRecord) then begin
        JobParams.retMsg := 'ERROR CarattAngle NOT Found "'+ strCarattAngle[ord(deg315)] +'" in dictionary CarattXRecords.';
        JobParams.retCode := resERROR;
        LogStep(578,578,578,578,  JobParams.retMsg );
        exit;
    end;
    ...
    if not CarattXRecords.TryGetValue( deg45, carattRecord) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(579,579,579,579,  JobParams.retMsg );
        exit;
    end;
    ...
    if not CarattXRecords.TryGetValue( deg135, carattRecord) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(580,580,580,580,  JobParams.retMsg );
        exit;
    end;
    ...
    if not CarattXRecords.TryGetValue( deg225, carattRecord) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(581,581,581,581,  JobParams.retMsg );
        exit;
    end;
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retMsg := 'CAN_Clean CHref ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(582,582,582,582,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retMsg := 'CAN_Clean CHdut ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(583,583,583,583,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(584,584,584,584, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !

    // Poi Reset ALL nodes, casomai alcuni fossero già in Start mode.
    LogStep(585,585,585,585, 'RESET REFs e DUT...');
    if not CAN_SendNMT( CHref, cmdResetNode, '00h' ) then begin  // Reset ALL nodes!
        ...
        JobParams.retCode := resERROR;
        LogStep(586,586,586,586, lastCAN_ErrMsg);
        exit;
    end;
    // The Reset was successfully sent
    if CHdut.Handle <> CHref.Handle then
        if not CAN_SendNMT( CHdut, cmdResetNode, edtDutAddr.text ) then begin
            JobParams.retMsg := 'DUT ResetNode '+edtDutAddr.text+' ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(587,587,587,587, lastCAN_ErrMsg);
            exit;
        end;
    LogStep(588,588,588,588, 'ALL Nodes Reset Successfully');
    LogStep(589,589,589,589, 'FLUSH queue All Can CHANNELS...');
    dmBase.Wait(100);                        // msec. per sicurezza essendo reset di 3 dev.
    // clean boot frames.
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(590,590,590,590,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(591,591,591,591,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(592,592,592,592, 'OK FLUSH All CanCH Done.');  // preparato rx queue clean.
    ...
    if not CAN_UnlockParams( edtDUTaddr.text ) then begin
        ...
        JobParams.retMsg := 'DUT UnlockParams ERROR: '+ lastCAN_ErrMsg;
        JobParams.retCode := resERROR;
        LogStep(593,593,593,593,  JobParams.retMsg );
        exit; // unlock non riuscito, quindi non serve lock finally !
    end;
    LogStep(594,594,594,594, 'OK, DUT Unlock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    try
        // Sblocco area protetta 5555h del DUT appena effettuato.
        // Azzera coefficiente Err1 nel DUT as UNS32. (5555-26)
        Can.Command  := _360_Err1_;                   // Campo per Coefficiente di Errore1 del DUT.
        ...
        // The parameter was Written successfully.
        LogStep(595,595,595,595, #9#9'Set DUT param '+ Can.Command +' = '+ iErr1.toString +' ('+iErr1.ToHexString(4)+'h)');
        LogStep(596,596,596,596, 'OK, param Err1 > '+Can.Index+'-'+Can.Subidx+'h  STORED Successfully to DutID:'+ edtDUTaddr.text );
        ...
        if not dicCanCmds.TryGetValue( Can.Command, CanCmd ) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(597,597,597,597,  JobParams.retMsg );
            exit;
        end;
        ...
        // The parameter was Written successfully.
        LogStep(598,598,598,598, #9#9'Set DUT param '+ Can.Command +' = '+ iErr2.toString +' ('+iErr2.ToHexString(4)+'h)');
        LogStep(599,599,599,599, 'OK, param Err2 > '+Can.Index+'-'+Can.Subidx+'h  STORED Successfully to DutID:'+ edtDUTaddr.text );

        // Conservo il valore di Errore2, nel Dictionary dicCanCmds, associato al proprio Alias.
        if not dicCanCmds.TryGetValue( Can.Command, CanCmd ) then begin
            JobParams.retMsg := 'ERROR Alias NOT Found "'+ Can.Command +'" in dictionary CanCmds.';
            JobParams.retCode := resERROR;
            LogStep(600,600,600,600,  JobParams.retMsg );
            exit;
        end;
        ...
        if CAN_NodeSaveAll( edtDUTaddr.text ) then begin
            // The frame was successfully sent
            LogStep(601,601,601,601, 'CAN SaveParams command SENT Successfully to NodeID:'+ edtDUTaddr.text );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            JobParams.retMsg := 'DUT SaveParams ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(602,602,602,602, 'ERROR on Save Params of DUT:'+ edtDUTaddr.text );
            LogStep(603,603,603,603,  lastCAN_ErrMsg );
            exit;
        end;
        // Ok, niente msec. attesa perchè WR in eeprom già completato.
        ...
        if CAN_SendNMT( CHdut, cmdResetNode, edtDUTaddr.text ) then begin
            // The Reset was successfully sent
            LogStep(604,604,604,604, 'CAN ResetNode SENT Successfully to NodeID:'+ edtDUTaddr.text );
        end
        else begin
            // l' Errore è bloccante, quindi esco.
            JobParams.retMsg := 'DUT ResetNode ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(605,605,605,605, 'ERROR on ResetNode DUT:'+ edtDUTaddr.text );
            LogStep(606,606,606,606,  JobParams.retMsg );
            exit;
        end;
        // DUT reset & boot completed !
        // Read back cfg, non serve se WR senza errori...
        if not CAN_FlushRxBuffer(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            LogStep(607,607,607,607,  JobParams.retMsg );
            exit;
        end;
        LogStep(608,608,608,608, 'OK FLUSH Can queue Successful.');    // preparato rx queue clean.

        LogStep(609,609,609,609, 'OK, completato aggiornamento parametri Err1,Err2' );
        JobParams.retMsg := _Completato_;
        JobParams.retCode := resOK;
        result := True;
    finally
        ...
        if not CAN_LockParams( edtDUTaddr.text ) then begin
            ...
            JobParams.retMsg := 'DUT LockParams ERROR: '+ lastCAN_ErrMsg;
            JobParams.retCode := resERROR;
            LogStep(610,610,610,610, 'ERROR on LOCK params for DUT:'+ edtDUTaddr.text );
            LogStep(611,611,611,611,  lastCAN_ErrMsg );
        end
        else
            LogStep(612,612,612,612, 'OK, DUT Lock command CONFIRMED by NodeID:'+ edtDUTaddr.text );
    end;
end;
...
begin
{
    ...
    if not CAN_CleanCH(CHref) then begin
        ...
        JobParams.retCode := resERROR;
        inc(errCount);
        LogStep(613,613,613,613,  lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_CleanCH(CHdut) then begin
            ...
            JobParams.retCode := resERROR;
            inc(errCount);
            LogStep(614,614,614,614,  lastCAN_ErrMsg );
            exit;
        end;
    LogStep(615,615,615,615, JobList.Strings[currJob]+': STATUS PCAN_OK');
    // Channels are Ready to work, ma meglio stoppare eventuale cyclical transmission dal REF !

    // prepara rx queue clean
    LogStep(616,616,616,616, 'FLUSH queue All Can CHANNELS...');
    if not CAN_FlushRxBuffer(CHref) then begin
        LogStep(617,617,617,617,  'ERROR on CHref Flush: '+lastCAN_ErrMsg );
        exit;
    end;
    if CHdut.Handle <> CHref.Handle then
        if not CAN_FlushRxBuffer(CHdut) then begin
            LogStep(618,618,618,618,  'ERROR on CHdut Flush: '+lastCAN_ErrMsg );
            exit;
        end;
    LogStep(619,619,619,619, 'OK FLUSH All CanCH Successful.');    // preparato rx queue clean.
    // NB: ATTENZIONE a non fare altri Reset del node !!!
    //     perchè annulla i setup temporanei necessari (es. risoluzione in centesimi) ricaricando i default !
    ...
    if not CAN_SendNMT( CHref, cmdStartNode, edtRef1Addr.text ) then begin
        ...
        JobParams.retCode := resERROR;
        LogStep(620,620,620,620, JobParams.retMsg);
        exit;
    end;
    // The Start was successfully sent
    LogStep(621,621,621,621, 'OK StartNode SENT Successfully to REF1: '+ edtRef1Addr.text );
    ...
    try
        ...
        try
          while true do begin // resta in endless loop fino a validazione ok, o user abort.
              ...
              //LogStep(622,622,622,622, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) );
              // Se resetto subito il flag sono sicuro di non perdere il prossimo set da parte del timer.
              TInterlocked.BitTestAndClear(SampleReady, 2);     // Reset SampleReady bit 2 = from Ref 2 assi
              //LogStep(623,623,623,623, 'Flag SampleReady = '+ format('$%8.8x', [SampleReady]) +' =>> RELEASED');
              ...
              LogStep(624,624,624,624, 'REF_in_Range counters   X='+ xREFinRange_counter.toString );
              ...
              if not CAN_SendNMT( CHref, cmdStopNode, edtRef1Addr.text ) then begin
                  JobParams.retMsg := 'ERROR on StopNode REF1: '+ lastCAN_ErrMsg;
                  JobParams.retCode := resERROR;
                  LogStep(625,625,625,625,  lastCAN_ErrMsg );
                  inc(errCount);
                  exit;
              end;
              LogStep(626,626,626,626, 'OK REF node Stopped.');

              dmBase.Wait(25);   // lascia digerire i comandi ai nodes.
              LogStep(627,627,627,627, 'ALL Cyclic transmission stopped Successfully');
              // e svuota da eventuali residui
              if not CAN_FlushRxBuffer(CHref) then begin
                  JobParams.retMsg := 'ERROR on Flush CHref buffer: '+ lastCAN_ErrMsg;
                  JobParams.retCode := resERROR;
                  LogStep(628,628,628,628,  JobParams.retMsg );
                  inc(errCount);
                  exit;
              end;
              LogStep(629,629,629,629, 'OK FLUSH CHref queue Successful.');    // preparato rx queue clean.
              ...
              LogStep(630,630,630,630, 'Attesa letture campioni stabili da DUT...');
              dmBase.Wait(1500);
              ...
              for i := 1 to kDutSamplesStable do begin  // DutStable in 1,5 sec fa 5 reads.
                  ...
                  if not CAN_Read_ANGLE( edtDUTaddr.text, 'X', iDeg) then begin // Delay standard è 25 msec
                      JobParams.retMsg := 'DUT Read X-angle ERROR: '+ lastCAN_ErrMsg;
                      JobParams.retCode := resERROR;
                      LogStep(631,631,631,631,  JobParams.retMsg );
                      inc(errCount);
                      break;
                  end;
                  // The currXDut was Read successfully !
              ...
              if not DutStable then begin
                  ...
                  LogStep(632,632,632,632,  lastCAN_ErrMsg );
                  inc(errCount);
                  break;
              end;
              edtDutX.text := format('%6.2f', [currXDut]);  // currXDut.ToString;
...
//*
              ...
              if not CAN_Read_ANGLE( edtREF1addr.text, 'X', iDeg) then begin // Delay standard è 25 msec
                  ...
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(633,633,633,633,  JobParams.retMsg );
                  exit;
              end;
              // The currXRef was Read successfully !
              currXRef := iDeg / REF1_resolution;           // applico risoluzione del Ref.
...
//
            ...
            //ListItem.ImageIndex := -1;                  // per ora niente icon
              ...
              if Abs(DutERR_test) > DutList[currDutIdx].ErrOneAxis then begin      // Abs() perchè errore ammesso è +/-
                  ...
                  JobParams.retCode := resERROR;
                  inc(errCount);
                  LogStep(634,634,634,634,  JobParams.retMsg );
              end;
              break;      // infine esce sempre !!!
        ...
        tmrFasiBlink.Enabled := False;     // termina blink.
        img1Seq1.Visible := False;         //
        LogStep(635,635,635,635, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
      //TestRecord.free; No, si perde il record di test appena memorizzato!  che serve in report !
        CAN_SendNMT( CHref, cmdStopNode, edtRef1Addr.text ); // blind stop invio ciclico, se non è già a causa Abort.
        CAN_FlushRxBuffer(CHref);                            // più che altro per lasciare pulizia a steps successivi.
    end;
...
begin
    ...
    if not dmBase.pgConn.Connected then begin
        LogStep(636,636,636,636, 'ERRORE Database '+ dmBase.pgConn.Server +':'+ dmBase.pgConn.Database+' NON Connesso !');
    ...
    try (*
        ...
        *)
        ...
        dmBase.pgConn.StartTransaction;  // gestione transazione usando try-except !
        try
            LogStep(637,637,637,637, 'Salvataggio dati test di collaudo per S/N: '+ Sessione.Serial+'  su DB Produzione.' );
            ...
            with dmBase.qryFindSession do begin
                ...
                LogStep(638,638,638,638, 'Execute Query...');
                Open;
            ...
            LogStep(639,639,639,639, 'Query check Done.');
            ...
            with dmBase.tabSessionDut do begin
              ...
              *)
              CheckBrowseMode;
              Insert;
              LogStep(640,640,640,640, 'Insert master tabSessionDut...');
            ...
            LogStep(641,641,641,641, 'Insert detail Test Records...');
            ...
            LogStep(645,645,645,645, 'Insert detail tabDutParams Records...');
        ...
        LogStep(649,649,649,649, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
        if dmBase.pgConn.InTransaction then     // se trova pendenze impreviste
...
var
  ...
  begin
      LogStep(650,650,650,650, 'Build nome Report...');
      result := False;
      //rptFile := vReportPath +'\'+ Sessione.CodiceProdotto +'_'+ Sessione.Lotto +'\'+ Sessione.CodiceProdotto +'_EOL_'+ Sessione.Lotto + Sessione.Serial +'.xls';
...
begin
    ...
    try
        try
            LogStep(651,651,651,651, 'Generazione Report EOL per S/N: '+ Sessione.Serial );
            // Se siamo in versione FULL (2+1 assi), questo report 1Asse và aggiunto al file pre-esistente del precedente EOL a 2Assi !
{
            ...
            if not buildFileName() then begin       // genera file Name rptFile
                LogStep(652,652,652,652, JobParams.retMsg);
                inc(errCount);
                exit;                               // passa sempre per finally
            end;
            //rptFile := vReportPath +'\'+ Sessione.CodiceProdotto +'_'+ Sessione.Lotto +'\'+ Sessione.CodiceProdotto +'_EOL_'+ Sessione.Lotto + Sessione.Serial +'.xls';
            ...
            if dmBase.FindFile( rptFile ) then begin
                ...
                if not AnsiContainsText(DutList[currDutIdx].ModoMisura, 'full') then begin
                    ...
                    LogStep(653,653,653,653, 'INFO: Deleted 2 Axis unused sheet.');
                end;
            end;
            ...
            LogStep(654,654,654,654, 'Print session Master fields...');
            ...
            LogStep(655,655,655,655, 'Print Work times fields...');
            ...
            if i >= 0 then begin
                ...
                JobParams.retCode := resERROR;
                LogStep(656,656,656,656, JobParams.retMsg);
                exit;
            end;
            ...
            if i >= 0 then begin
                ...
                JobParams.retCode := resERROR;
                LogStep(657,657,657,657, JobParams.retMsg);
                exit;
            end;
            for i := aJobRecord.ListIndex to JobList.Count-1 do begin  // e da li proseguo scorrendo tutti i restanti steps,
            ...
            LogStep(658,658,658,658, 'Print calibration table...');
            rowOffset := 3;
            // stampa Titolo
            ...
            LogStep(659,659,659,659, 'Print Caratterizzazioni...');
            ...
            LogStep(660,660,660,660, 'Print TestsX table...');
            rowOffset := 18;
            // stampa tipo asse
            ...
            LogStep(661,661,661,661, 'Print CAN Parameters table...');
            (*
            ...
            *)
            ...
            LogStep(662,662,662,662, 'Creazione Report FILE rev.'+ iRev.ToString);
            try
                // Salva file as XLS
            ...
            if errCount = 0 then begin
                LogStep(664,664,664,664, 'OK Report EOL compilato e salvato in:');
                LogStep(665,665,665,665,  rptFile );
        ...
        Screen.Cursor := crDefault;
        LogStep(667,667,667,667, JobList.Strings[currJob]+' ERRORS = '+ intToStr(errCount));
        dmBase.wait(500);// anche solo per evitare click multipli involontari da parte user.
    end;
end;
